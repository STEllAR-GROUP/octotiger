...
int main(int argc,char** argv) {
	try {
		...
		auto const ref=ReferenceData::read(argv[1]); auto const& p=ref.p;
		double const h=p.length/ref.n;
		// Exact mode limits: k=0; chi=0; critical damping; huge chi with finite slow decay.
		auto m=telegraphMode(0,.7,3,2); near(m[0],1,0,"zero mode");
		m=telegraphMode(12,.2,1,0); near(m[0],std::cos(.4),2e-15,"undamped mode");
		m=telegraphMode(3,.7,1,2); near(m[0],1.7*std::exp(-.7),2e-15,"critical damping");
		m=telegraphMode(1,1,1,1e12); near(m[0],1-1./3e12,2e-15,"overdamped stability");
		for (int d=0;d<3;++d) {
			Point x{.12,-.43,.2}, y=x; y[d]+=p.length;
			auto a=streamingWave(x,.37,h,p), b=streamingWave(y,.37,h,p);
			for (int f=0;f<4;++f) near(a[f],b[f],2e-14,"oblique wave periodicity");
		}
		Parameters units=p; units.c=17;
		auto beam=streamingFront({0,0,0},0,h,units);
		near(beam[1],17*beam[0],1e-14,"physical front flux");
		near(beam[0],.5+.5e-10,1e-14,"front cell average");
		auto center=equilibriumSphere({0,0,0},p);
		...
		for(int d=0;d<3;++d) {
			...
			auto a=equilibriumSphere(lo,p), b=equilibriumSphere(hi,p);
			div+=(b[d+1]-a[d+1])/(2*eps);
			near((b[0]-a[0])/(2*eps),-3*p.chi/p.c*equilibriumSphere(x,p)[d+1],1e-9,"diffusion gradient");
		}
		...
		// Evolve E,F with 4-variable RK4 rather than reusing telegraphMode.
		...
		rejected=false;try {ReferenceData::read(bad);} catch(std::runtime_error const&){rejected=true;}
