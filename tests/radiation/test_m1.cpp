    ...
    for (Real e : {1e-20, 1e-6, 1.0, 1e12, 1e30}) {
        for (Real ratio : {1e-20, 1e-4, 1.0, 1e4, 1e20}) {
            for (Real eta : {1e-14, 1e-3, 1.0, 1e8, 1e20}) {
                ...
                const Real logAlpha=std::log(2*E)-4*std::log(e);
                const auto next=rm::thermal_exchange(e,E,eta,logAlpha);
                ...
                const long double B=std::exp((long double)logAlpha+4*std::log((long double)next.gas));
...
void coupledSources() {
    ...
    require(zero.radiation==u && zero.momentum==momentum && zero.gasEnergy==e+K,"zero-coupling identity");
    for (const auto& opac : {std::pair{0.,0.7}, std::pair{0.7,0.7}, std::pair{0.2,0.7}}) {
        ...
        near(out.radiation[0]+out.gasEnergy,u[0]+e+K,2e-13,"coupled total energy conserved");
        ...
        near(out.gasEnergy-kinetic,out.internalEnergy,2e-13,"thermal energy includes kinetic change");
        admissible(out.radiation,c);
    }
    const Real chi=2,dt=0.03;
    const auto staticGas=rm::couple(u,{0,0,0},e,e,rho,chi,chi,dt,c,std::log(4.)-4*std::log(e));
    for(int d=0;d<3;++d) {
        near(staticGas.radiation[d+1],u[d+1]/(1+dt*c*chi),1e-14,"implicit flux damping");
        near(staticGas.momentum[d],(u[d+1]-staticGas.radiation[d+1])/c/c,1e-14,"physical momentum conversion c squared");
    }
    const auto stiff=rm::couple({1,0,0,0},{0,0,0},100,100,1,1e15,1e15,1,c,-4*std::log(100.));
    near(stiff.radiation[0]+stiff.gasEnergy,101,2e-13,"stiff coupled energy conserved");
    admissible(stiff.radiation,c);
}

void periodicTransport() {
    ...
    for(int i=0;i<N;++i) {
        Real E=1+0.2*std::sin(2*piR*(i+0.5)*dx);
        u[i]={E,c*E,0,0};
    }
    const auto initial=u;
    const int steps=static_cast<int>(std::ceil((1/c)/rm::transportTimestep(dx,c,0.4)));
    ...
    near(rm::transportTimestep(2,10,0.4),1./60,1e-14,"3D light CFL bound");
    near(rm::transportTimestep(2,10,0.4,10),1./120,1e-14,"moving-grid CFL bound");
}
...
int main() {
    try {
        closureAndHll(); reconstruction(); thermalSources(); coupledSources(); periodicTransport(); debugGuards();
