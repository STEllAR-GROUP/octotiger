...
std::size_t init_thread_local_worker(std::size_t desired) {
	...
	if (current == desired) {
		...
		mono_inter_p2p::stencil_masks() = mono_inter::calculate_stencil_masks(mono_inter_p2p::stencil()).first;
		mono_inter_p2p::four() = mono_inter::calculate_stencil().second;
		mono_inter_p2p::stencil_four_constants() = mono_inter::calculate_stencil_masks(mono_inter_p2p::stencil()).second;
		...
		multi_inter_p2p::stencil_masks() = multi_inter::calculate_stencil_masks(multi_inter_p2p::stencil()).first;
		multi_inter_p2p::inner_stencil_masks() = multi_inter::calculate_stencil_masks(multi_inter_p2p::stencil()).second;
...
//	grid::set_unit_conversions();
		...
		std::array<size_t, 6> analyze_local_launch_counters() {
			...
			std::cout << "CPU multipole launches on locality " << hpx::get_locality_id() << ": "
					<< total_multipole_cpu_launches << std::endl;
			std::cout << "CUDA multipole launches on locality " << hpx::get_locality_id() << ": "
					<< total_multipole_cuda_launches << std::endl;
			if (total_multipole_cpu_launches + total_multipole_cuda_launches > 0) {
				...
				std::cout << "=> Percentage of multipole on the GPU on locality " << hpx::get_locality_id() << ": "
						<< percentage * 100 << "\n";
			}
			std::cout << "----------------------------------------" << std::endl;
			...
			std::cout << "CPU non-rho-multipole launches on locality " << hpx::get_locality_id() << ": "
					<< total_multipole_cpu_launches_non_rho << std::endl;
			std::cout << "CUDA non-rho-multipole launches on locality " << hpx::get_locality_id() << ": "
					<< total_multipole_cuda_launches_non_rho << std::endl;
			if (total_multipole_cpu_launches_non_rho + total_multipole_cuda_launches_non_rho > 0) {
				...
				std::cout << "=> Percentage of non-rho-multipole on the GPU on locality " << hpx::get_locality_id() << ": "
						<< percentage * 100 << "\n";
			}
			std::cout << "----------------------------------------" << std::endl;
			std::cout << "Total p2p launches on locality " << hpx::get_locality_id() << ": "
					<< total_p2p_cpu_launches + total_p2p_cuda_launches << std::endl;
			std::cout << "CPU p2p launches on locality " << hpx::get_locality_id() << ": " << total_p2p_cpu_launches
					<< std::endl;
			std::cout << "CUDA p2p launches on locality " << hpx::get_locality_id() << ": " << total_p2p_cuda_launches
					<< std::endl;
		...
		void accumulate_distributed_counters() {
			...
			std::cout << "========================================" << std::endl;
			std::cout << "========================================" << std::endl;
			std::cout << "Total multipole launches: " << total_multipole_cpu_launches + total_multipole_cuda_launches
					<< std::endl;
			...
			std::cout << "========================================" << std::endl;
			std::cout << "Total p2p launches: " << total_p2p_cpu_launches + total_p2p_cuda_launches << std::endl;
				...
				if (opts().process_options(argc, argv)) {
					...
					if (opts().problem != AMR_TEST) {
						hpx::async(&node_server::execute_solver, root,
								opts().problem == DWD && opts().restart_filename.empty(), ngrids).get();
					} else {
						root->enforce_bc();
