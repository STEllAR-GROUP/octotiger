...
void init_executors(void) {
	...
	if (num_devices > 0) { // some devices were found
		if (opts().number_gpus > num_devices) {
			std::cerr << "ERROR: Requested " << opts().number_gpus << " GPUs but only " << num_devices << " were found!" << std::endl;
			abort();
		}
...
#ifdef OCTOTIGER_HAVE_KOKKOS
	Kokkos::initialize();
	if (hpx::get_locality_id() == 0) Kokkos::print_configuration(std::cout, true);
	...
	std::cout << "CPPuddle config: Using " << recycler::number_instances << " internal buffer buckets!" << std::endl;
#if defined(KOKKOS_ENABLE_CUDA)
	stream_pool::set_device_selector<hpx::kokkos::cuda_executor, round_robin_pool<hpx::kokkos::cuda_executor>>([](size_t gpu_id) {
		cudaSetDevice(gpu_id);
	});
	// initialize stencils / executor pool in kokkos device
	for (size_t gpu_id = 0; gpu_id < opts().number_gpus; gpu_id++) {
		stream_pool::init_executor_pool<hpx::kokkos::cuda_executor, round_robin_pool<hpx::kokkos::cuda_executor>>(
			gpu_id, opts().executors_per_gpu, hpx::kokkos::execution_space_mode::independent);
	}
	std::cout << "KOKKOS/CUDA is enabled!" << std::endl;
#elif defined(KOKKOS_ENABLE_HIP)

	stream_pool::set_device_selector<hpx::kokkos::hip_executor, round_robin_pool<hpx::kokkos::hip_executor>>([](size_t gpu_id) {
		hipSetDevice(gpu_id);
	});
	for (size_t gpu_id = 0; gpu_id < opts().number_gpus; gpu_id++) {
		stream_pool::init_executor_pool<hpx::kokkos::hip_executor, round_robin_pool<hpx::kokkos::hip_executor>>(
			gpu_id, opts().executors_per_gpu, hpx::kokkos::execution_space_mode::independent);
	}
	std::cout << "KOKKOS/HIP is enabled!" << std::endl;
#elif defined(KOKKOS_ENABLE_SYCL)
	for (size_t gpu_id = 0; gpu_id < opts().number_gpus; gpu_id++) {
		stream_pool::init_executor_pool<hpx::kokkos::sycl_executor, round_robin_pool<hpx::kokkos::sycl_executor>>(
			gpu_id, opts().executors_per_gpu, hpx::kokkos::execution_space_mode::independent);
	}
	...
	octotiger::fmm::monopole_interactions::get_device_masks<device_buffer<int>, host_buffer<int>, kokkos_device_executor>(mover);
	octotiger::fmm::monopole_interactions::get_device_constants<device_buffer<double>, host_buffer<double>, kokkos_device_executor>(mover);
	octotiger::fmm::multipole_interactions::get_device_masks<device_buffer<int>, host_buffer<int>, kokkos_device_executor>(mover, true);
	get_flux_device_masks<device_buffer<bool>, host_buffer<bool>, kokkos_device_executor>(mover);
	Kokkos::fence();
#if HPX_KOKKOS_CUDA_FUTURE_TYPE == 0
	...
	stream_pool::set_device_selector<hpx::cuda::experimental::cuda_executor, round_robin_pool<hpx::cuda::experimental::cuda_executor>>(
		[](size_t gpu_id) {
			cudaSetDevice(gpu_id);
		});
#if HPX_KOKKOS_CUDA_FUTURE_TYPE == 0
	...
	for (size_t gpu_id = 0; gpu_id < opts().number_gpus; gpu_id++) {
		stream_pool::init_executor_pool<hpx::cuda::experimental::cuda_executor, pool_strategy>(gpu_id, opts().executors_per_gpu, gpu_id,
																							   true);
	}
#else
...
void init_problem(void) {
	...
	} else if (opts().problem == RADIATION_STREAMING_FRONT) {
		set_analytic(analyticStreamingFront);
		set_problem(testStreamingFront);
		set_refine_test(radiation_test_refine);
	} else if (opts().problem == RADIATION_STREAMING_WAVE) {
		set_analytic(analyticStreamingWave);
		set_problem(testStreamingWave);
		set_refine_test(radiation_test_refine);
	} else if (opts().problem == RADIATION_GAUSSIAN_PULSE) {
		set_analytic(analyticGaussianPulse);
		set_problem(testGaussianPulse);
		set_refine_test(radiation_test_refine);
	} else if (opts().problem == RADIATION_EQUILIBRIUM_SPHERE) {
		set_analytic(analyticEquilibriumSphere);
		set_problem(testEquilibriumSphere);
		set_refine_test(radiation_test_refine);
	...
	if (OCTOTIGER_MAX_NUMBER_FIELDS > physics<NDIM>::nf_) {
		std::cerr << "\nWarning! OCTOTIGER_WITH_MAX_NUMBER_FIELDS too large for this scenario!" << std::endl
				  ...
				  << "Choose -DOCTOTIGER_WITH_MAX_NUMBER_FIELDS=" << physics<NDIM>::nf_ << " to run this scenario with optimal performance"
				  << std::endl
				  << std::endl;
		...
		std::cerr << "\nERROR! OCTOTIGER_WITH_MAX_NUMBER_FIELDS too small for this scenario!" << std::endl
				  << "Recompile with -DOCTOTIGER_WITH_MAX_NUMBER_FIELDS=" << physics<NDIM>::nf_ << std::endl
				  << std::endl;
		exit(EXIT_FAILURE);
	} else {
...
void register_cppuddle_allocator_counters(void) {
	...
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<double, boost::alignment::aligned_allocator<double, 32>>);
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<int, boost::alignment::aligned_allocator<int, 32>>);
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<double, std::allocator<double>>);
	hpx::register_startup_function(&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<int, std::allocator<int>>);

	// CUDA host / device allocators -- also used by KOKKOS
#if defined(OCTOTIGER_HAVE_CUDA)
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<double, recycler::detail::cuda_pinned_allocator<double>>);
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<int, recycler::detail::cuda_pinned_allocator<int>>);
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<double, recycler::detail::cuda_device_allocator<double>>);
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<int, recycler::detail::cuda_device_allocator<int>>);
#endif
	// HIP host / device allocators -- also used by KOKKOS
#if defined(OCTOTIGER_HAVE_HIP)
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<double, recycler::detail::hip_pinned_allocator<double>>);
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<int, recycler::detail::hip_pinned_allocator<int>>);
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<double, recycler::detail::hip_device_allocator<double>>);
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<int, recycler::detail::hip_device_allocator<int>>);
#endif
	// SYCL host / device allocators
#if defined(OCTOTIGER_HAVE_KOKKOS) && defined(KOKKOS_ENABLE_SYCL)
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<double, detail::sycl_host_default_allocator<double>>);
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<int, detail::sycl_host_default_allocator<int>>);
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<double, detail::sycl_device_default_allocator<double>>);
	hpx::register_startup_function(
		&recycler::detail::buffer_recycler::register_allocator_counters_with_hpx<int, detail::sycl_device_default_allocator<int>>);
#endif
