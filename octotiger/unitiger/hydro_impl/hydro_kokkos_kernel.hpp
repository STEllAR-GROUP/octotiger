...
constexpr int f_inx3 = (INX + 1) * (INX + 1) * (INX + 1);
    ...
    if (agg_exec.sync_aggregation_slices()) {
        ...
        Kokkos::parallel_for(
            "kernel hydro flux", policy, KOKKOS_LAMBDA(int idx) {
                ...
                const int f_slice_offset = (NDIM * nf * f_inx3 + padding) * slice_id;
                ...
                auto f_combined_slice = Kokkos::subview(f_combined,
                    std::make_pair(
                        f_slice_offset, (NDIM * nf * f_inx3 + padding) *
                        (slice_id + 1)));
        ...
        Kokkos::parallel_for(
            "kernel hydro flux", policy, KOKKOS_LAMBDA(const membertype& team_handle) {
                ...
                auto f_combined_slice = Kokkos::subview(
                    f_combined, std::make_pair(f_slice_offset, (NDIM * nf * f_inx3 + padding) *
                    (slice_id + 1)));
...
void reconstruct_teamless_impl(
    ...
    if (agg_exec.sync_aggregation_slices()) {
        ...
        auto policy = Kokkos::Experimental::require(
            ...
            Kokkos::Experimental::WorkItemProperty::HintLightWeight);
        Kokkos::parallel_for(
            "kernel hydro reconstruct_teamless", policy,
...
timestep_t device_interface_kokkos_hydro(
    ...
    Allocator_Slice<int, kokkos_host_allocator<int>, executor_t>& alloc_host_int, auto& F_flat, const auto& U) {
    ...
    for (int f = 0; f < nf; f++) {
        kokkos_um_pinned_array_const<real> vector_wrapper_u(U[f].data(), H_N3);
        auto u_current_slice = Kokkos::subview(u,
            std::make_pair<size_t, size_t>(
                f * H_N3 + u_slice_offset * slice_id, f * H_N3 + u_slice_offset * slice_id + H_N3));

        deep_copy(agg_exec.get_underlying_executor().instance(), u_current_slice, vector_wrapper_u);
        /* std::copy( */
    ...
    aggregated_device_buffer<double, executor_t> f(
        alloc_device_double, (NDIM * nf * f_inx3 + padding) * max_slices);
...
timestep_t launch_hydro_kokkos_kernels(const hydro_computer<NDIM, INX, physics<NDIM>>& hydro,
    const auto& U, const std::vector<std::vector<safe_real>>& X,
    ...
    auto ret_fut = executor_slice_fut.value().then(hpx::annotated_function([&](auto && fut) {
      ...
      max_lambda = device_interface_kokkos_hydro(combined_x, combined_large_x,
          disc_detect, smooth_field, geo.NDIR, hydro.get_nf(),
          ...
          physics<NDIM>::de_switch_1, agg_exec, alloc_host_double, alloc_host_int, F_flat, U);
      ...
      /*                   F[dim][field].data()); */
              ...
              /*           /1* const auto input_index = *1/ */
              /*           /1*     (i + 1) * q_inx * q_inx + (j + 1) * q_inx + 1; *1/ */
