...
template <typename T>
CUDA_GLOBAL_METHOD inline void complete_hydro_amr_boundary_inner_loop(const double dx,
    const bool energy_only, const T* __restrict__ unified_ushad, const int* __restrict__ coarse,
    const T* __restrict__ xmin, T* __restrict__ unified_uf, const int i0, const int j0,
    const int k0, const int nfields) {
                ...
                for (int kr = 0; kr < 2; kr++) {
                    for (int f = 0; f < nfields; f++) {
                        if (!energy_only || f == egas_i) {
                            ...
                            uf += (1.0 / 64.0) * s_xyz;
                            store_value<T>(unified_uf, f * field_offset + 8 * iii0 + oct_index, uf);
                        }
                    }
                    if (!energy_only) {
                        const int oct_index = ir * 4 + jr * 2 + kr;
                        ...
                        const double x = (i1) *dx + xmin[XDIM];
                        const double y = (j1) *dx + xmin[YDIM];
                        const double z = (k1) *dx + xmin[ZDIM];

                        const T sx_val =
                            load_value<T>(unified_uf, sx_i * field_offset + 8 * iii0 + oct_index);
                        const T sy_val =
                            load_value<T>(unified_uf, sy_i * field_offset + 8 * iii0 + oct_index);
                        const T sz_val =
                            load_value<T>(unified_uf, sz_i * field_offset + 8 * iii0 + oct_index);

                        T lx_val =
                            load_value<T>(unified_uf, lx_i * field_offset + 8 * iii0 + oct_index);
                        lx_val -= y * sz_val - z * sy_val;
                        store_value<T>(
                            unified_uf, lx_i * field_offset + 8 * iii0 + oct_index, lx_val);

                        T ly_val =
                            load_value<T>(unified_uf, ly_i * field_offset + 8 * iii0 + oct_index);
                        ly_val += x * sz_val - z * sx_val;
                        store_value<T>(
                            unified_uf, ly_i * field_offset + 8 * iii0 + oct_index, ly_val);

                        T lz_val =
                            load_value<T>(unified_uf, lz_i * field_offset + 8 * iii0 + oct_index);
                        lz_val -= x * sy_val - y * sx_val;
                        store_value<T>(
                            unified_uf, lz_i * field_offset + 8 * iii0 + oct_index, lz_val);
                    }
                }
            }
        }
            ...
            for (int ir = 0; ir < 2; ir++) {
                for (int jr = 0; jr < 2; jr++) {
                    for (int kr = 0; kr < 2; kr++) {
                        ...
                        zy *= factor;
                        zz *= factor;
                        store_value<T>(unified_uf, lx_i * field_offset + 8 * iii0 + oct_index, zx);
                        store_value<T>(unified_uf, ly_i * field_offset + 8 * iii0 + oct_index, zy);
                        store_value<T>(unified_uf, lz_i * field_offset + 8 * iii0 + oct_index, zz);
                    }
                }
            }
            for (int ir = 0; ir < 2; ir++) {
