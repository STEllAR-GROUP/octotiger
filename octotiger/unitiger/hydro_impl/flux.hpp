...
#pragma ivdep
			for (int f = 0; f < nf_; f++) {
				...
				if (this_ap - this_am != 0.0) {
					this_flux[f] = (this_ap * FL[f] - this_am * FR[f] + this_ap * this_am * (UR[f] - UL[f]))
							/ (this_ap - this_am);
...
template<int NDIM, int INX, class PHYS>
timestep_t hydro_computer<NDIM, INX, PHYS>::flux_experimental(const hydro::recon_type<NDIM> &Q, hydro::flux_type &F,
	hydro::x_type &X, safe_real omega) {
