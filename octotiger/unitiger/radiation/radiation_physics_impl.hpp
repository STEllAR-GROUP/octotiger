...
template<int NDIM>
template<int INX>
void radiation_physics<NDIM>::physical_flux(const oct::vector<safe_real> &U, oct::vector<safe_real> &F, int dim, safe_real &am, safe_real &ap,
	...
	const safe_real *fr = &U[fx_i];
	oct::array<double, NDIM> n;
	oct::array<double, NDIM> T;
...
void radiation_physics<NDIM>::pre_angmom(const hydro::state_type &U, const hydro::recon_type<NDIM> &Q,
		oct::array<safe_real, cell_geometry<NDIM, INX>::NANGMOM> &Z, oct::array<oct::array<safe_real, cell_geometry<NDIM, INX>::NDIR>, NDIM> &S, int i,
		safe_real dx) {
	static const cell_geometry<NDIM, INX> geo;
...
template<int NDIM>
template<int INX>
void radiation_physics<NDIM>::post_recon(oct::vector<oct::vector<oct::vector<safe_real>>> &Q, const hydro::x_type X, safe_real omega, bool angmom) {
	static const cell_geometry<NDIM, INX> geo;
	const auto dx = X[0][geo.H_DNX] - X[0][0];
...
template<int NDIM>
template<int INX>
oct::vector<typename hydro_computer<NDIM, INX, radiation_physics<NDIM>>::bc_type> radiation_physics<NDIM>::initialize(radiation_physics<NDIM>::test_type t,
		hydro::state_type &U, hydro::x_type &X) {
	static const cell_geometry<NDIM, INX> geo;

	oct::vector<typename hydro_computer<NDIM, INX, radiation_physics<NDIM>>::bc_type> bc(2 * NDIM);
