...
//		printf( "%i | %i %i %i | ", flip_dim, d, dims[0], dims[1]);
	...
	std::vector<std::array<safe_real, geo::NDIR / 2>> fluxes(nf);
	std::array < safe_real, 3 > amax = { 0.0, 0.0, 0.0 };
	static const auto indices3 = find_indices(3, geo::H_NX - 2);
	for (int dim = 0; dim < NDIM; dim++) {
		...
		for (const auto &i : indices3) {
			...
			for (int f = 0; f < nf; f++) {
				F[dim][f][i] = 0.0;
				for (int fi = 0; fi < geo::NFACEDIR; fi++) {
					F[dim][f][i] += weights[fi] * fluxes[f][fi];
				}
			}
				...
				for (int n = 0; n < geo::NANGMOM; n++) {
					for (int m = 0; m < NDIM; m++) {
						for (int l = 0; l < NDIM; l++) {
							for (int fi = 0; fi < geo::NFACEDIR; fi++) {
								const auto d = faces[dim][fi];
								F[dim][zx_i + n][i] += weights[fi] * kdelta[n][m][l] * face_loc[d][m] * 0.5 * dx * fluxes[sx_i + l][fi];
							}
						}
					}
				}
