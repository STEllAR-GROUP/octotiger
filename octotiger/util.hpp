...
template<class ... Args>
int stdout_printf(const char* str, Args&&...args) {
	if (opts().disable_stdout) {
		FILE* fp = fopen("stdout.txt", "at");
		if (fp == NULL) {
			return -1;
		}
		fprintf(fp, str, std::forward<Args>(args)...);
...
bool find_root(std::function<real(real)>& func, real xmin, real xmax, real& root, real toler = 1.0e-10);
