...
namespace octotiger { namespace radiation {
    ...
    void radiation_kernel(integer const d, std::vector<real> const& rho,
        ...
        std::vector<real> const& mmw, std::vector<real> const& X_spc,
        std::vector<real> const& Z_spc, real dt, real const clightinv)
    {
        return radiation_cpu_kernel<er_i, fx_i, fy_i, fz_i>(d, rho, sx, sy, sz,
            egas, tau, fgamma, U, mmw, X_spc, Z_spc, dt, clightinv);
    }
