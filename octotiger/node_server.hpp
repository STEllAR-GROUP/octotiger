...
private:
	 ...
	 *  of the 6 faces, 12 edges, and 8 vertices of the subgrid cube. If there is an AMR boundary to a coarser level, that neighbor is
	 * empty. */
	 ...
	 * face direction there are 4 adjacent neighbors (or zero). This is used for AMR boundary handling - interpolation onto finer boundaries
	 * and flux matchinig.*/
...
private:
	...
	void collect_hydro_boundaries(Real t, bool energy_only = false);
...
public:
	...
	void all_hydro_bounds(Real t);
	void energy_hydro_bounds(Real t);
	...
	node_count_type regrid_gather(bool rebalance_only); /**/
	...
	int form_tree(hpx::id_type, hpx::id_type = hpx::invalid_id,
				  std::vector<hpx::id_type> = std::vector<hpx::id_type>(geo::direction::count())); /**/
...
public:
	...
	void all_rad_bounds(Real t);

	void collect_radiation_bounds(Real t);
