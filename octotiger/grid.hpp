...
// HPX_IS_BITWISE_SERIALIZABLE(analytic_t);  commenting this line because it causes failures when
// futures return analytic_t type from one locality to another
...
void line_of_centers_analyze(const line_of_centers_t& loc, real omega,
    std::pair<real, real>& rho1_max, std::pair<real, real>& rho2_max, std::pair<real, real>& l1_phi,
    std::pair<real, real>& l2_phi, std::pair<real, real>& l3_phi, real& rho1_phi, real& rho2_phi);
...
class grid {
    constexpr static auto intBox = Box<NDIM>(H_BW, H_NX - H_BW);
    constexpr static auto extBox = Box<NDIM>(0, H_NX);
    constexpr static auto intBox2 = intBox.scale(2);
    constexpr static auto extBox2 = extBox.scale(2);
    constexpr static auto intBoxShadow = Box<NDIM>(H_BW, INX / 2 + H_BW);
    constexpr static auto extBoxShadow = Box<NDIM>(0, INX / 2 + 2 * H_BW);
    constexpr static auto intBoxFlux = Box<NDIM>(INX);
    constexpr static auto extBoxFlux = Box<NDIM>(INX + 1);
    constexpr static auto intBoxRefine = Box<NDIM>(H_BW - REFINE_BW, H_NX - H_BW + REFINE_BW);
    constexpr static auto extBoxRefine = extBox;
...
private:
    ...
    void compute_boundary_interactions_monopole_monopole(gsolve_type type,
        const std::vector<boundary_interaction_type>&, const gravity_boundary_type&);
...
public:
    ...
    void compute_boundary_interactions(
        gsolve_type, const geo::direction&, bool is_monopole, const gravity_boundary_type&);
    ...
    bool is_in_star(const std::pair<space_vector, space_vector>& axis,
        const std::pair<real, real>& l1, integer frac, integer index, real rho_cut) const;
    ...
    std::vector<real> get_flux_restrict(
        std::array<integer, NDIM> lb, std::array<integer, NDIM> ub, const geo::dimension&) const;
    std::vector<real> get_prolong(
    ...
    std::vector<real> get_subset(
        const std::array<integer, NDIM>& lb, const std::array<integer, NDIM>& ub, bool energy_only);
    ...
    void diagnostics();
    real z_moments(const std::pair<space_vector, space_vector>& axis,
        const std::pair<real, real>& l1, integer frac, real rho_cut) const;
    std::vector<real> frac_volumes() const;
    real roche_volume(const std::pair<space_vector, space_vector>& axis,
        const std::pair<real, real>& l1, real, bool donor) const;
