...
def _application_command(opts, levels, build, output, source_root, case):
    ...
    return radiation_results.command(source_root, mode, case, arguments)


def _run_application(command, source_root):
    return subprocess.run(command, cwd=source_root, check=False,
                          env=os.environ.copy()).returncode


def _write_progress(out, results, runner, site):
    statuses = [record['status'] for record in results]
    summary = {
        'source_commit': runner.git_value('rev-parse', 'HEAD'),
        'families': results,
...
def execute(selected, arguments, plan=False):
    ...
    if application:
        ...
        command = _application_command(opts, levels, build, out/'radiation'/'application',
                                       runner.SOURCE_ROOT, case)
    ...
    results = []
    components = []
    _write_progress(out, results, runner, site)
    ...
    for family, component, callback, inputs, args in groups:
        ...
        family_records = [record for record in components if record['family'] == family]
        results = [record for record in results if record['family'] != family]
        results.append({'family': family, 'status': _family_status(family_records),
                        'components': family_records})
        _write_progress(out, results, runner, site)
        try:
            if component == 'application':
                print('+ ' + ' '.join(map(str, args)), flush=True)
                code = _run_application(args, runner.SOURCE_ROOT)
            else:
            ...
            running.update(returncode=1, status='failed', reason=repr(error))
        family_records = [record for record in components if record['family'] == family]
        results = [record for record in results if record['family'] != family]
        results.append({'family': family, 'status': _family_status(family_records),
                        'components': family_records})
