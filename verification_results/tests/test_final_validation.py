...
class FinalValidationTests(unittest.TestCase):
    def test_scenario_failure_is_visible_and_outputs_are_isolated(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(scenario.subprocess, 'run', side_effect=fake), patch.object(runner, 'gitValue', return_value='test'):
            ...
            self.assertEqual(result['input_options']['mesh.level.maximum'], '1')
    ...
    def test_missing_executable_records_options_and_non_success(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('gravity.enabled=off', result['config_text'])
    ...
    def test_output_reuse_and_source_output_rejected(self):
        ...
        with self.assertRaises(runner.HarnessError): scenario.execute([], ['--output', str(runner.sourceRoot)])

    def test_mixed_suite_aggregates_failures_and_distinct_outputs(self):
        outputs = []
        def fakeScenario(inputs, args, **kw):
            outputs.append(runner.optionValue(args, '--output'))
            self.assertNotIn('2', args)
            return 1 if inputs[0][1]['family'] == 'hydro' else 3
        def fakeNative(inputs, args, **kw):
            outputs.append(runner.optionValue(args, '--output'))
            self.assertEqual(args[:3], ['0', '1', '2'])
            return 0
        with tempfile.TemporaryDirectory() as tmp, patch.object(scenario, 'execute', side_effect=fakeScenario), patch.object(native_suite, 'execute', side_effect=fakeNative), patch.object(unified, 'runApplication', return_value=0) as application:
    ...
    def test_scenario_metadata_matches_inputs_and_schema_contract(self):
        for _, d in runner.descriptors().values():
            ...
            if d['adapter']['name'] == 'octotiger_scenario':
                config = (runner.sourceRoot/d['parameters']['config']).read_text()
                self.assertIn('mesh.level.maximum='+str(d['resolution_levels'][0]), config)
        ...
        self.assertIn(pattern, (runner.sourceRoot/'test_problems/sphere/CMakeLists.txt').read_text())
