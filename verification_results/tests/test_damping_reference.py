...
from verification_results.radiation.damping_reference import discrete_mean, damping_checks
...
class DampingReferenceTests(unittest.TestCase):
    ...
    def test_backward_euler_is_first_order_without_altering_reference(self):
        ...
        for n in (32, 64, 128):
            h, p = self.history(n)
            self.assertTrue(damping_checks(h, p)['backward_euler_mean'])
    ...
    def test_wrong_source_or_step_history_is_rejected(self):
        ...
        self.assertFalse(damping_checks(h, p)['backward_euler_mean'])
    ...
    def test_variable_steps_and_input_validation(self):
        ...
        h['mean_E'] = discrete_mean(h, p)
        self.assertTrue(damping_checks(h, p)['backward_euler_mean'])
        h['rad_dt'][2] = 0
        with self.assertRaises(ValueError): discrete_mean(h, p)
