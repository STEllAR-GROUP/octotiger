...
class DescriptorTests(unittest.TestCase):
    ...
    def test_adapter_preserves_legacy_case_and_arguments(self):
        command = radiation_results.command(
            runner.sourceRoot, "live", "streaming_wave", ["2", "3", "Release", "--threads", "7"]
        )
        self.assertEqual(command[1:3], ["live", "streaming_wave"])
        self.assertEqual(command[3:5], ["--root", str(runner.sourceRoot)])
    ...
    def test_adapter_project_root_matches_legacy_layouts(self):
        ...
        flatCommand = radiation_results.command(flat, "run", "streaming_wave", [])
        nestedCommand = radiation_results.command(nested, "run", "streaming_wave", [])
        self.assertEqual(flatCommand[3:5], ["--root", str(flat)])
        self.assertEqual(nestedCommand[3:5], ["--root", "/work/project"])
        self.assertEqual(radiation_results.projectRoot(flat), flat)
        self.assertEqual(radiation_results.projectRoot(nested), Path("/work/project"))
    ...
    def test_metadata_contract_and_stable_paths(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = radiation_results.command(runner.sourceRoot, "run", case, arguments)
    ...
    def test_source_input_output_is_rejected(self):
        with self.assertRaises(runner.HarnessError):
            runner.safeOutput(runner.testRoot / "accidental-output")
        with self.assertRaises(runner.HarnessError):
            runner.safeOutput(runner.harnessRoot / "references" / "accidental-output")
        with self.assertRaises(runner.HarnessError):
            runner.safeOutput(runner.sourceRoot / "src" / "accidental-output")

    def test_adapter_defaults_match_legacy_modes(self):
        case, path, descriptor = runner.resolve("wave")
        output = Path("/tmp/verification-default-contract")
        command = radiation_results.command(runner.sourceRoot, "run", case, [])
        runMeta = runner.metadata("run", "wave", path, descriptor, [], output, command)
        liveMeta = runner.metadata("live", "wave", path, descriptor, [], output, command)
        self.assertEqual(runMeta["execution"]["resolution_levels"], [2, 3])
        self.assertEqual(runMeta["build"]["build_type"], "Release")
        self.assertEqual(liveMeta["execution"]["resolution_levels"], [2, 3, 4])
        self.assertEqual(liveMeta["build"]["build_type"], "Debug")
    ...
    def test_value_options_do_not_become_levels_or_build(self):
        ...
        command = radiation_results.command(runner.sourceRoot, "live", case, arguments)
        value = runner.metadata(
...
class LauncherTests(unittest.TestCase):
    def test_launcher_works_outside_checkout(self):
        launcher = runner.harnessRoot / "run.sh"
        with tempfile.TemporaryDirectory() as directory:
