...
installed already. Put this `verification_results/radiation` directory under
`~/octotiger/src/octotiger`. The launchers treat `~/octotiger` as the project
root, `~/octotiger/src/octotiger` as the source checkout, and
`~/octotiger/build/octotiger/$TYPE` as the application build directory.

From `~/octotiger`:

```bash
src/octotiger/verification_results/radiation/results.sh install
src/octotiger/verification_results/radiation/run.sh all 2 3 4 release
```
...
```bash
bash verification_results/radiation/run.sh all 2 3 4 release --movies
```
...
```bash
bash verification_results/radiation/run.sh wave 2 3 4 release
bash verification_results/radiation/run.sh streaming 2 3 4 debug
bash verification_results/radiation/run.sh diffusion 2 3 4 release --no-silo
bash verification_results/radiation/run.sh sphere 2 3 4 release --threads 8
```
...
Use `--root /path/to/project` if the project directory is elsewhere. Use
`--build RelWithDebInfo` for `PROJECT/build/octotiger/RelWithDebInfo`, and
...
```bash
src/octotiger/verification_results/radiation/run.sh all 2 3 release --output verification_results/radiation/results/check1
src/octotiger/verification_results/radiation/run.sh diffusion 2 3 release --time 0.4 --odt 0.1
```
...
```bash
bash verification_results/radiation/run.sh all 2 3 4 release --dry-run
```
...
```bash
python3 verification_results/radiation/run_live.py sphere 2 release --threads 12 \
...
It is idempotent and refuses conflicting files. Use `python3 verification_results/radiation/install.py --check .` from the checkout
...
```bash
python3 -m unittest discover -s verification_results/radiation/tests -v
```
