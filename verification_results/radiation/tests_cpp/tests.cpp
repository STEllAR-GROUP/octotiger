...
void results(const fs::path &p, Json m)
{
	...
	for (int f = 0; f < 4; ++f)
		log << fields[f] << ' ' << (f == 0 ? error : 0) << ' ' << (f == 0 ? 2 * error : 0) << ' '
			<< (f == 0 ? 4 * error : 0) << '\n';
	atomicText(p / "run.log", log.str());
	Json analytic = Json::object();
	for (int f = 0; f < 4; ++f)
		analytic[fields[f]] = {{"l1", f == 0 ? error : 0}, {"l2", f == 0 ? 2 * error : 0},
			{"linf", f == 0 ? 4 * error : 0}};
	writeJson(p / "runSummary.json",
		{{"schemaVersion", 1}, {"status", "completed"}, {"finalTime", time}, {"analytic", analytic},
			{"units", {{"grams", 1}, {"centimeters", 1}, {"seconds", 1}, {"lightSpeed", cCgs}}}});
	for (int k = 0; k < 3; ++k)
		atomicText(p / (norms[k] + ".dat"), number(dx) + " " + std::to_string(m.at("level").get<int>()) +
	...
	atomicText(p / "radiation-slices/slice-000.csv", csv.str());
	...
	for (double t : {0., time / 4, time})
		budget << t << ',' << volume << ',' << volume * (1 + t) << ",0,0,0,0,0,0,0," << volume * t
			   << ",0,0,0\n";
	atomicText(p / "radiation-conservation.csv", budget.str());
}
void silos(const fs::path &folder, const Json &m)
{
	...
	for (auto entry : std::vector<std::pair<std::string, double>>{
		...
		require(DBPutMultimesh(db, "mesh", 2, mesh_names, meshTypes, opt) >= 0, "fixture multimesh failed");
		char meshName[] = "mesh";
		DBAddOption(opt, DBOPT_MMESH_NAME, meshName);
		for (auto &f : fields) {
...
{
	try {
		auto op = opacityMetadata({{"radiation.opacity.model", "grey"},
		...
		expect(op.at("material_model") == "grey" && op.at("absorption") == "2" &&
				...
				op.at("prescribed_test_chi_code_inverse_length") == "5",
			"opacity metadata roles");
		expect(opacityMetadata({{"rad_opacity", "0.7"}}).at("legacy_constant") == "0.7",
		...
		if (argc > 1 && std::string(argv[1]) == "--fake-visit") {
			...
			require(readText(opts.at("-sessionfile")).find("Pseudocolor_1.0") != std::string::npos,
					"Missing session argument");
			const char *modeEnv = std::getenv("RR_TEST_VISIT_MODE");
		...
		if (argc > 1 && std::string(argv[1]).starts_with("--runtime.config_file=")) {
		...
		{
			auto original = readText(p / "radiation-conservation.csv");
			auto small = m;
			small["length"] = 2.;
			small["c"] = 4.;
			atomicText(p / "radiation-conservation.csv", original.substr(0, original.find('\n') + 1) +
															  "0,8,4,-2,0,1.6e-15,0,0,0,0,0,0,0,0\n"
															  "4,8,45,-1,4,8,0,0.5,0,0,40,0.25,2,0\n");
			auto fixed = *readConservation(p, small);
			...
			atomicText(p / "radiation-conservation.csv", original);
		}
		verifyCgsSummary(p / "runSummary.json");
		auto cap = cadence(4, 61, .4);
		expect(cap["steps_per_output_check"] == 5 && close(cap["hard_dt"], 4. / 300),
			   "capture output-check timestep cap");
		auto seq = frameSchedule({0, .25, 1}, 4, 2, .5);
		expect(seq == std::vector<std::size_t>({0, 0, 0, 1, 1, 1, 2, 2}),
			   "nonuniform physical-time schedule");
		rejects([] { frameSchedule({0, 0, 1}, 4, 2, .5); }, "duplicate time rejected");
		auto csv = readText(p / "radiation-slices/slice-000.csv");
		atomicText(p / "radiation-slices/slice-001.csv", csv);
		rejects([&] { readSlice(p, m); }, "duplicate cells rejected");
		fs::remove(p / "radiation-slices/slice-001.csv");
		auto summary = readJson(p / "runSummary.json");
		auto incompleteSummary = summary;
		incompleteSummary["status"] = "running";
		writeJson(p / "runSummary.json", incompleteSummary);
		rejects([&] { readNorms(p, m); }, "incomplete structured summary rejected");
		writeJson(p / "runSummary.json", summary);
		auto wrong = m;
		wrong["time"] = 5.;
		rejects([&] { readNorms(p, wrong); }, "stale final time rejected");
		rejects([&] { readConservation(p, wrong); }, "stale conservation history rejected");
		auto data = readText(p / "radiation-conservation.csv");
		atomicText(p / "radiation-conservation.csv", data + "nan,1\n");
		rejects([&] { readConservation(p, m); }, "malformed budget rejected");
		atomicText(p / "radiation-conservation.csv", data);
		silos(p, m);
		for (auto axis : {"x", "y", "z"}) {
			Options o;
			o.axis = axis;
			auto s = readSilo(p / "X.1.silo", o, m);
		...
		Options o;
		o.field = "fluxmag";
		auto s = readSilo(p / "X.0.silo", o, m);
		for (auto &c : s.cells) {
			double value = 2 + (c.lo[0] + c.hi[0]) / 2 / 6e10 + 2 * (c.lo[1] + c.hi[1]) / 2 / 6e10 +
						   3 * (c.lo[2] + c.hi[2]) / 2 / 6e10;
			expect(close(c.value, std::sqrt(29.) * value), "flux magnitude");
		}
		expect(numericalSilos(p).size() == 4, "numerical snapshot listing");
		Options sessions;
		sessions.sessionDir = tmp / "flat sessions";
		auto firstSession = makeSession(p, sessions);
		auto xml = readText(firstSession.at("session_file").get<std::string>());
		expect(firstSession.at("frames").size() == 3 &&
				   fs::path(firstSession["frames"].back()["silo"].get<std::string>()).filename() ==
					   "final.silo",
			   "session deduplicates final state and prefers numerical final.silo");
		expect(firstSession["frames"][0]["time"] == 0 && firstSession["frames"][1]["time"] == 1 &&
				   firstSession["frames"][2]["time"] == 4,
			   "session retains irregular recorded times");
		auto again = makeSession(p, sessions);
		expect(again["session_file"] == firstSession["session_file"], "stable session filename on repeat");
		sessions.position = 1e9;
		auto shifted = makeSession(p, sessions);
		expect(shifted["session_file"] != firstSession["session_file"],
			   "distinct slice positions cannot collide");
		expect(fs::path(shifted["session_file"].get<std::string>()).parent_path() == sessions.sessionDir,
		...
		auto gaussianMeta = m;
		gaussianMeta["case"] = "gaussian_pulse";
		results(p, gaussianMeta);
		sessions.position = 0;
		auto gaussianSession = makeSession(p, sessions);
		auto gaussianXml = readText(gaussianSession["session_file"].get<std::string>());
		expect(gaussianXml.find("max(er-(1),") != std::string::npos &&
				   gaussianXml.find(">\"Log\"</Field>") != std::string::npos,
			   "Gaussian session clips energy excess and uses logarithmic scaling");
		expect(gaussianSession["color_limits"][0].get<double>() > 0, "Gaussian log floor is positive");
		sessions.view = "3d";
		sessions.field = "fluxmag";
		auto surface = makeSession(p, sessions);
		auto surfaceXml = readText(surface["session_file"].get<std::string>());
		expect(surfaceXml.find("sqrt(fx*fx+fy*fy+fz*fz)") != std::string::npos &&
				   surfaceXml.find("Slice_1.0") == std::string::npos,
			   "3D magnitude session retains physical flux expression without slicing");
		writeJson(p / "run.json", m);

		atomicText(tmp / "hash.txt", "abc");
