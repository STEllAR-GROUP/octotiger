...
void output_stage1(std::string fname, int cycle) {
	...
	for (auto i = node_registry::begin(); i != node_registry::end(); ++i) {
		...
		if (!node_ptr_->refined()) {
			futs_.push_back(hpx::async(HPX_PRIORITY_BOOST [](node_location loc, node_registry::node_ptr ptr) {
...
//							}
	...
	if (this_id < ge - 1) {
		auto f = hpx::async<output_stage3_action>(HPX_PRIORITY_BOOST localities[this_id + 1], fname, cycle, gn, gb, ge);
...
void output_all(node_server *root_ptr, std::string fname, int cycle, bool block) {
	...
	for (auto &id : localities) {
		futs1.push_back(hpx::async<output_stage1_action>(HPX_PRIORITY_BOOST id, fname, cycle));
	}
	GET(hpx::when_all(futs1));

	std::vector<hpx::future<node_list_t>> id_futs;
	for (auto &id : localities) {
		id_futs.push_back(hpx::async < output_stage2_action > (HPX_PRIORITY_BOOST id, fname, cycle));
	}
...
//		printf( "---%i\n", id) ;
	...
	for (int i = 0; i < ng; i++) {
		...
		futs.push_back(hpx::async < output_stage3_action > (HPX_PRIORITY_BOOST localities[gb], fname, cycle, i, gb, ge));
	}

	barrier = hpx::async(HPX_PRIORITY_BOOST [tstart, fname, cycle](std::vector<hpx::future<void>> &&futs) {
		for (auto &f : futs) {
			GET(f);
		}
