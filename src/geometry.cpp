...
face octant::get_face(const dimension& d) const {
	const integer o = *this;
	return face(2 * d + ((o >> d) & 1));
}

void octant::set(integer x, integer y, integer z) {
	integer i;
	i = (x == 1) ? 1 : 0;
	i |= (y == 1) ? 2 : 0;
	i |= (z == 1) ? 4 : 0;
...
void octant::set(double x, double y, double z) {
	integer i;
	i = (x >= 0.0) ? 1 : 0;
	i |= (y >= 0.0) ? 2 : 0;
	i |= (z >= 0.0) ? 4 : 0;
...
integer get_boundary_size(std::array<integer, NDIM>& lb, std::array<integer, NDIM>& ub, const geo::direction& dir, const geo::side& side, integer bw) {
	integer hsize, size;
	size = 0;
