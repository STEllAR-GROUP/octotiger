...
grid::output_list_type node_server::load(integer cnt, const hpx::id_type &_me,
  ...
  if (rec_size == -1 && my_location.level() == 0) {
    ...
    for (auto &locality : localities) {
      futs.push_back(hpx::async<set_locality_data_action>(locality, omega,
                                                          pivot, rec_size));
    }
...
hpx::future<grid::output_list_type>
node_client::output(std::string fname, int cycle, bool flag) const {
  ...
  if (is_refined) {
    ...
    if (refinement_flag != 0) {
      ...
      for (auto &ci : geo::octant::full_set()) {
        ...
        children[ci] =
            hpx::new_<node_server>(hpx::find_here(), my_location.get_child(ci),
                                   me, current_time, rotational_time);
        ...
        if (current_time > ZERO) {
          children[ci]
              .set_grid(grid_ptr->get_prolong(lb, ub), std::move(outflows))
              .get();
        }
      }
    }
...
hpx::future<void> node_client::regrid_scatter(integer a, integer b) const {
  return hpx::async<typename node_server::regrid_scatter_action>(get_gid(), a,
                                                                 b);
}
