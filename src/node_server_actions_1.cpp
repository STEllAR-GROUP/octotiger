...
struct flock file_write_lock(FILE* fp) {
	int fd = fileno(fp);
...
struct flock file_read_lock(FILE* fp) {
	int fd = fileno(fp);
	static struct flock lock;
	lock.l_type = F_RDLCK;
	lock.l_start = 0;
...
void file_unlock(FILE* fp, struct flock& lock) {
	int fd = fileno(fp);
	lock.l_type = F_UNLCK;
...
hpx::future<grid::output_list_type> node_client::load(integer i, integer total, integer rec_size, bool do_o,
		std::string s) const {
	return hpx::async<typename node_server::load_action>(get_unmanaged_gid(), i, total, rec_size, do_o, s);
}

hpx::future<grid::output_list_type> node_server::load(integer cnt, integer total_nodes, integer rec_size, bool do_output,
		std::string filename) {
...
//		}).get();
		...
		for (auto const& ci : geo::octant::full_set()) {
			...
			futs[index++] =
					hpx::new_ < node_server
							> (options::all_localities[loc_id], my_location.get_child(ci), me.get_gid(), ZERO, ZERO, step_num, hcycle, gcycle).then(
	...
	return hpx::dataflow([this, do_output, filename](std::array<hpx::future<grid::output_list_type>, NCHILD>&& futs)
	{
...
hpx::future<grid::output_list_type> node_client::output(std::string dname, std::string fname, int cycle, bool flag) const {
		...
		if (my_location.level() == 0) {
			...
			if (opts.parallel_silo) {
				...
				for (integer i = 0; i != sz; ++i) {
					...
					futs[i] = hpx::async < parallel_output_complete_action
							> (opts.all_localities[i], this_dname, this_fname, get_time(), cycle, analytic);
				}
				hpx::wait_all(futs);
	...
	if (is_refined) {
		...
		for (auto& ci : geo::octant::full_set()) {
			...
			if (children[ci].empty()) {
				futs[index++] = create_child(child_loc, ci).then([this, ci, a, total](hpx::future<hpx::id_type>&& child)
				{
