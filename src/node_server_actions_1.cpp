...
node_count_type node_server::regrid(const hpx::id_type& root_gid, real omega, real new_floor, bool rb,bool grav_energy_comp) {
	...
	stdout_printf("-----------------------------------------------\n");
	if (!rb) {
		stdout_printf("checking for refinement\n");
		check_for_refinement(omega, new_floor);
	} else {
		node_registry::clear();
	}
	stdout_printf("regridding\n");
	real tstart = timer.elapsed();
	auto a = regrid_gather(rb);
	real tstop = timer.elapsed();
	stdout_printf("Regridded tree in %f seconds\n", real(tstop - tstart));
	stdout_printf("rebalancing %i nodes with %i leaves\n", int(a.total), int(a.leaf));
	tstart = timer.elapsed();
	regrid_scatter(0, a.total);
	tstop = timer.elapsed();
	stdout_printf("Rebalanced tree in %f seconds\n", real(tstop - tstart));
	assert(grid_ptr != nullptr);
	tstart = timer.elapsed();
	stdout_printf("forming tree connections\n");
	a.amr_bnd = form_tree(hpx::unmanaged(root_gid));
	stdout_printf( "%i amr boundaries\n", a.amr_bnd);
	tstop = timer.elapsed();
	stdout_printf("Formed tree in %f seconds\n", real(tstop - tstart));
	stdout_printf("solving gravity\n");
	solve_gravity(grav_energy_comp, !opts().output_filename.empty());
	double elapsed = timer.elapsed();
	stdout_printf("regrid done in %f seconds\n---------------------------------------\n", elapsed);
	return a;
}
...
void node_server::set_aunt(const hpx::id_type& aunt, const geo::face& face) {
	if (aunts[face].get_gid() != hpx::invalid_id) {
		stdout_printf("AUNT ALREADY SET\n");
		abort();
	}
	aunts[face] = aunt;
}
