...
const real M_acc = 1.0;
const real M_don = 0.2;
const real R_don = 0.02;
	...
	for (integer i = H_BW; i != H_NX - H_BW; ++i) {
		for (integer j = H_BW; j != H_NX - H_BW; ++j) {
			for (integer k = H_BW; k != H_NX - H_BW; ++k) {
				...
				phi = G[phi_i][iiig];
				bool is_don = U[frac0_i][iii] < U[frac1_i][iii];
				...
				if (U[frac0_i][iii] > scf_data.rho_max_acc && !is_don) {
					scf_data.phi_eff_acc = phi_eff;
					scf_data.rho_max_acc = rho;
					scf_data.x_acc = x;
				}
				if (U[frac1_i][iii] > scf_data.rho_max_don && is_don) {
...
//	w0 = MIN;
	...
	for (integer i = 0; i != 1001; ++i) {
		...
		//	if (done) {
				...
				SYSTEM(std::string("./octotiger -problem=dwd -restart=X.chk -output=X.") + std::to_string(i) + std::string(".silo"));
		//	}
