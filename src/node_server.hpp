...
private:
	...
	integer step_num;
	real current_time;
	std::shared_ptr<grid> grid_ptr;bool is_refined;
	...
	std::array<std::shared_ptr<channel<std::vector<real>>> ,NFACE> sibling_hydro_channels;
	...
	std::array<std::shared_ptr<channel<std::pair<std::vector<real>, bool>>> ,NFACE> sibling_gravity_channels;
...
public:
	...
	node_server(const node_location&, integer, bool, real, const std::array<integer,NCHILD>&, grid, const std::vector<hpx::id_type>&);
...
//	static grid::output_list_type output_collect(const std::string&);
	...
	std::list<hpx::future<void>> set_nieces_amr(integer) const;
	node_server();
	~node_server();
	...
	void set_gravity_boundary(const std::vector<real>&, integer face, bool monopole);
	...
	void regrid(const hpx::id_type& root_gid);
...
//	HPX_DEFINE_COMPONENT_ACTION(node_server, get_sibling_clients, get_sibling_clients_action);
	...
	void set_aunt(const hpx::id_type&,  integer face );
	HPX_DEFINE_COMPONENT_ACTION(node_server, set_aunt, set_aunt_action);

	std::vector<hpx::id_type> get_nieces(const hpx::id_type&,  integer face ) const;
