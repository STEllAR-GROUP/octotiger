...
//			}
		...
		if (!opts().disable_output) {
			hpx::threads::run_as_os_thread([=]() {
				...
				fprintf(fp, "%i %e %e %e %e %e %e %e %e %i %i %i %i\n", int(next_step - 1), double(t), double(dt_.dt), time_elapsed, rotational_time, dt_.x, dt_.y, dt_.z, dt_.a, dt_.dim, int(ngrids.total), int(ngrids.leaf), int(ngrids.amr_bnd));
				fclose(fp);
		...
		hpx::threads::run_as_os_thread(
				[=]() {
					printf("%i %e %e %e %e %e %e %e %e %i %i %i %i\n", int(next_step - 1), double(t), double(dt_.dt), time_elapsed, rotational_time, dt_.x,
							dt_.y, dt_.z, dt_.a, dt_.dim, int(ngrids.total), int(ngrids.leaf), int(ngrids.amr_bnd));
