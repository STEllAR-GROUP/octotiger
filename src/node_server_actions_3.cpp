...
//			}
		...
		if (!opts().disable_output) {
			hpx::threads::run_as_os_thread([=]() {
				...
				fprintf(fp, "%i %e %e %e %e %e %e %e %e %i %i %i\n", int(next_step - 1), double(t), double(dt_), time_elapsed, rotational_time, theta, theta_dot, omega, omega_dot, int(ngrids.total), int(ngrids.leaf), int(ngrids.amr_bnd));
				fclose(fp);
			});     // do not wait for it to finish
		}

		hpx::threads::run_as_os_thread([=]() {
			printf("%i %e %e %e %e %e %e %e %e\n", int(next_step - 1), double(t), double(dt_), time_elapsed, rotational_time, theta, theta_dot, omega, omega_dot);
