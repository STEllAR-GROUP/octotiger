...
void rad_imp_comoving(real& E, real& e, const std::function<real(real, real)>& kp, const std::function<real(real, real)>& B, real rho, real dt) {
//	printf("%e %e\n", e, E);
...
//		printf( "Error in rad_imp_comoving\n");
		...
		const real dkp_de = (kp(rho, e + de) - kp(rho, e)) * 0.5 / de;
		const real dB_de = (B(rho, e + de) - B(rho, e)) * 0.5 / de;
		f = (E - E0) + dt * c * kp(rho, e) * (E - 4.0 * M_PI / c * B(rho, e));
		const real dfdE = 1.0 + dt * c * kp(rho, e);
		const real dfde = dt * c * dkp_de * (E - 4.0 * M_PI / c * B(rho, e)) - dt * kp(rho, e) * 4.0 * M_PI * dB_de;
		dE = -f / (dfdE - dfde);
		const real w = 0.6;
	...
	for (integer i = R_BW; i != R_NX - R_BW; ++i) {
		for (integer j = R_BW; j != R_NX - R_BW; ++j) {
			for (integer k = R_BW; k != R_NX - R_BW; ++k) {
				...
				const real beta_2 = v2 * cinv * cinv;
				...
				rad_imp_comoving(E0, e0, kappa_p, B_p, rho[iiih], dt);
...
void node_server::compute_radiation(real dt) {
	...
	integer nsteps = std::min(int(std::ceil(dt / max_dt)), 1);
	//if( nsteps % 2 == 1 ) {
	//	++nsteps;
//	}
	...
	for (integer xi = 0; xi != R_NX; ++xi) {
		for (integer yi = 0; yi != R_NX; ++yi) {
			...
			//	sigma_a[iiir] = J[iiir] = rho[iiih];
				J[iiir] = kappa_p(rho[iiih], e) * B_p(rho[iiih], e);
