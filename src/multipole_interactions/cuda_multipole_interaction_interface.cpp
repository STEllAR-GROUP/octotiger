...
namespace fmm {
    namespace multipole_interactions {
        ...
        // }
        template <typename T>
        __device__ void compute_kernel_rho(T (&X)[NDIM], T (&Y)[NDIM], T (&m_partner)[20],
        ...
        template <typename T>
        __device__ void compute_kernel_non_rho(T (&X)[NDIM], T (&Y)[NDIM], T (&m_partner)[20],
    ...
    __global__ void compute_multipole_interactions(void) {}
    }    // namespace multipole_interactions
}    // namespace fmm
}    // namespace octotiger
#endif
