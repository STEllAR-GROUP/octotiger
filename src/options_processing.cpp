...
constexpr double mass_solar = 1.2969;
constexpr double number_solar = 1.0994;
constexpr double X_solar = 0.7068;
constexpr double Z_solar = 0.0181;

inline std::string to_string(const std::string &str) {
	return str;
}

inline std::string to_string(const double &num) {
...
bool options::process_options(int argc, char *argv[]) {
	...
	command_opts.add_options()																							  //
		("help", "produce help message")("xscale", po::value<double>(&(opts().xscale))->default_value(1.0), "grid scale") //
		("dt_max", po::value<double>(&(opts().dt_max))->default_value(0.333333), "max allowed pct change for positive fields in a timestep")															   //
		("cfl", po::value<double>(&(opts().cfl))->default_value(0.4), "cfl factor")												   //
		("omega", po::value<double>(&(opts().omega))->default_value(0.0), "(initial) angular frequency")						   //
		...
		("eblast0", po::value<double>(&(opts().eblast0))->default_value(1.0), "energy for blast wave")							   //
		("rho_floor", po::value<double>(&(opts().rho_floor))->default_value(0.0), "density floor")								   //
		("tau_floor", po::value<double>(&(opts().tau_floor))->default_value(0.0), "entropy tracer floor")						   //
		("sod_rhol", po::value<double>(&(opts().sod_rhol))->default_value(1.0), "density in the left part of the grid")			   //
		("sod_rhor", po::value<double>(&(opts().sod_rhor))->default_value(0.125), "density in the right part of the grid")		   //
		("sod_pl", po::value<double>(&(opts().sod_pl))->default_value(1.0), "pressure in the left part of the grid")			   //
		...
		("solid_sphere_radius", po::value<double>(&(opts().solid_sphere_radius))->default_value(1.0 / 3.0), "radius of the sphere") //
		("solid_sphere_mass", po::value<double>(&(opts().solid_sphere_mass))->default_value(1.0), "total mass enclosed inside the sphere") //
		("solid_sphere_rho_min", po::value<double>(&(opts().solid_sphere_rho_min))->default_value(1.0e-12), "minimal density outside (and within) the sphere")																 //
		("star_xcenter", po::value<double>(&(opts().star_xcenter))->default_value(0.0), "x-position of the star center") //
		("star_ycenter", po::value<double>(&(opts().star_ycenter))->default_value(0.0), "y-position of the star center") //
		("star_zcenter", po::value<double>(&(opts().star_zcenter))->default_value(0.0), "z-position of the star center") //
		("star_n", po::value<double>(&(opts().star_n))->default_value(1.5), "polytropic index of the star")				 //
		("star_rmax", po::value<double>(&(opts().star_rmax))->default_value(1.0 / 3.0), "maximal star radius")			 //
		("star_dr", po::value<double>(&(opts().star_dr))->default_value(1.0 / (3.0 * 128.0)), "differential radius for solving the Lane-Emden equation") //
		("star_alpha", po::value<double>(&(opts().star_alpha))->default_value(1.0 / (3.0 * 3.65375)), "scaling factor for the Lane-Emden equation") // for default n=3/2, ksi_1=3.65375 and alpha=rmax/ksi_1
		("star_rho_center", po::value<double>(&(opts().star_rho_center))->default_value(1.0), "density at the center of the star") //
		("star_rho_out", po::value<double>(&(opts().star_rho_out))->default_value(1.0e-10), "density outside the star")			   //
		("star_egas_out", po::value<double>(&(opts().star_egas_out))->default_value(1.0e-10), "gas energy outside the star")	   //
		("moving_star_xvelocity", po::value<double>(&(opts().moving_star_xvelocity))->default_value(1.0), "velocity of the star in the x-direction") //
		("moving_star_yvelocity", po::value<double>(&(opts().moving_star_yvelocity))->default_value(1.0), "velocity of the star in the y-direction") //
		("moving_star_zvelocity", po::value<double>(&(opts().moving_star_zvelocity))->default_value(1.0), "velocity of the star in the z-direction")																					 //
		("clight_retard", po::value<double>(&(opts().clight_retard))->default_value(1.0), "retardation factor for speed of light")	 //
		("driving_rate", po::value<double>(&(opts().driving_rate))->default_value(0.0), "angular momentum loss driving rate")		 //
		("driving_time", po::value<double>(&(opts().driving_time))->default_value(0.0), "A.M. driving rate time")					 //
		("entropy_driving_time", po::value<double>(&(opts().entropy_driving_time))->default_value(0.0), "entropy driving rate time") //
		("entropy_driving_rate", po::value<double>(&(opts().entropy_driving_rate))->default_value(0.0), "entropy loss driving rate") //
		("future_wait_time", po::value<int>(&(opts().future_wait_time))->default_value(-1), "")										 //
		("silo_offset_x", po::value<int>(&(opts().silo_offset_x))->default_value(0), "")											 //
		("silo_offset_y", po::value<int>(&(opts().silo_offset_y))->default_value(0), "")											 //
		("silo_offset_z", po::value<int>(&(opts().silo_offset_z))->default_value(0), "")											 //
		("amrbnd_order", po::value<int>(&(opts().amrbnd_order))->default_value(1), "amr boundary interpolation order")				 //
		("scf_output_frequency", po::value<int>(&(opts().scf_output_frequency))->default_value(25), "Frequency of SCF output")		 //
		("scf_rho_floor", po::value<double>(&(opts().scf_rho_floor))->default_value(1.0e-12), "scf density floor")					 //
		("silo_num_groups", po::value<int>(&(opts().silo_num_groups))->default_value(-1), "Number of SILO I/O groups")				 //
		("core_refine", po::value<bool>(&(opts().core_refine))->default_value(false), "refine cores by one more level")				 //
		("grad_rho_refine", po::value<double>(&(opts().grad_rho_refine))->default_value(-1.0), "density gradient refinement criteria (-1=off)")																		//
		("accretor_refine", po::value<int>(&(opts().accretor_refine))->default_value(0), "number of extra levels for accretor") //
		("extra_regrid", po::value<int>(&(opts().extra_regrid))->default_value(0), "number of extra regrids on startup")		//
		("donor_refine", po::value<int>(&(opts().donor_refine))->default_value(0), "number of extra levels for donor")			//
		("ngrids", po::value<int>(&(opts().ngrids))->default_value(-1), "fix numbger of grids")									//
		("refinement_floor", po::value<double>(&(opts().refinement_floor))->default_value(1.0e-3), "density refinement floor")	//
		("theta", po::value<double>(&(opts().theta))->default_value(0.5), "controls nearness determination for FMM, must be between 1/3 and 1/2")				   //
		("eos", po::value<eos_type>(&(opts().eos))->default_value(IDEAL), "gas equation of state") //
		("ipr_nr_tol", po::value<double>(&(opts().ipr_nr_tol))->default_value(1.48e-08), "Newton-Raphson tolerance for solving ideal gas plus radiation eos") //
		("ipr_nr_maxiter", po::value<int>(&(opts().ipr_nr_maxiter))->default_value(50), "Newton-Raphson max iterations for solving ideal gas plus radiation eos")														//
		("ipr_test", po::value<bool>(&(opts().ipr_test))->default_value(false), "test consistency of the ideal gas plus radiation eos") //
		("ipr_eint_floor", po::value<double>(&(opts().ipr_eint_floor))->default_value(0.0), "floor thermal energy for ideal gas plus radiation eos")													  //
		...
		("odt", po::value<double>(&(opts().output_dt))->default_value(1.0 / 100.0), "output frequency")						  //
		("dual_energy_sw1", po::value<double>(&(opts().dual_energy_sw1))->default_value(0.001), "dual energy switch 1")		  //
		("dual_energy_sw2", po::value<double>(&(opts().dual_energy_sw2))->default_value(0.1), "dual energy switch 2")		  //
		("hard_dt", po::value<double>(&(opts().hard_dt))->default_value(-1), "timestep size")								  //
		...
		("stop_time", po::value<double>(&(opts().stop_time))->default_value(std::numeric_limits<double>::max()), "time to end simulation") //
		("stop_step", po::value<int>(&(opts().stop_step))->default_value(std::numeric_limits<int>::max() - 1), "number of timesteps to run")																				//
		("min_level", po::value<int>(&(opts().min_level))->default_value(1), "minimum number of refinement levels") //
		("max_level", po::value<int>(&(opts().max_level))->default_value(1), "maximum number of refinement levels") //
		...
		("n_species", po::value<int>(&(opts().n_species))->default_value(5), "number of mass species")					//
		("atomic_mass", po::value<std::vector<double>>(&(opts().atomic_mass))->multitoken(), "atomic masses")			//
		("atomic_number", po::value<std::vector<double>>(&(opts().atomic_number))->multitoken(), "atomic numbers")		//
		("code_to_g", po::value<double>(&(opts().code_to_g))->default_value(1), "code units to grams")					//
		("code_to_cm", po::value<double>(&(opts().code_to_cm))->default_value(1), "code units to centimeters")			//
	...
	if (opts().theta < octotiger::fmm::THETA_FLOOR) {
		std::cerr << "theta " << theta << " is too small since Octo-Tiger was compiled for a minimum of " << octotiger::fmm::THETA_FLOOR
				  << std::endl;
...
#define SHOW(opt) std::cout << std::string(#opt) << " = " << to_string(opt) << '\n';
	...
	if (opts().gravity) {
		...
		if (opts().monopole_host_kernel_type == interaction_host_kernel_type::VC) {
			std::cerr << std::endl << "ERROR: ";
			std::cerr << "Octotiger has been compiled without Vc support!" << " Choose a different --monopole_host_kernel_type!"
					  << std::endl;
			abort();
		}
		if (opts().multipole_host_kernel_type == interaction_host_kernel_type::VC) {
			std::cerr << std::endl << "ERROR: ";
			std::cerr << "Octotiger has been compiled without Vc support! " << "Choose a different --multipole_host_kernel_type!"
					  << std::endl;
			abort();
		}
#endif
		...
		if (opts().monopole_host_kernel_type == interaction_host_kernel_type::KOKKOS) {
			std::cerr << std::endl << "ERROR: ";
			std::cerr << "Octotiger has been compiled without Kokkos support!" << " Choose a different --monopole_host_kernel_type!"
					  << std::endl;
			abort();
		}
		if (opts().multipole_host_kernel_type == interaction_host_kernel_type::KOKKOS) {
			std::cerr << std::endl << "ERROR: ";
			std::cerr << "Octotiger has been compiled without Kokkos support! " << " Choose a different --multipole_host_kernel_type!"
					  << std::endl;
			abort();
		}
#endif
		...
		if (opts().monopole_device_kernel_type == interaction_device_kernel_type::CUDA) {
			std::cerr << std::endl << "ERROR: ";
			std::cerr << "Octotiger has been compiled without CUDA support!" << " Choose a different --monopole_device_kernel_type!"
					  << std::endl;
			abort();
		}
		if (opts().multipole_device_kernel_type == interaction_device_kernel_type::CUDA) {
			std::cerr << std::endl << "ERROR: ";
			std::cerr << "Octotiger has been compiled without CUDA support! " << " Choose a different --multipole_device_kernel_type!"
					  << std::endl;
			abort();
		}
#endif
	...
	if (opts().eos == IPR) {
		if ((opts().hydro_host_kernel_type != interaction_host_kernel_type::VC) &&
			(opts().hydro_host_kernel_type != interaction_host_kernel_type::LEGACY)) {
			std::cerr << std::endl << "ERROR: ";
			std::cerr
				<< "The ideal gas plus radiation (ipr) eos is currently only supported with LEGACY host kernel types of the hydro solver!"
				<< std::endl
	...
	if (opts().executors_per_gpu < 1 && (opts().monopole_device_kernel_type != OFF || opts().multipole_device_kernel_type != OFF ||
										 opts().hydro_device_kernel_type != OFF)) {
		std::cerr << std::endl << "ERROR: ";
	...
	if (opts().max_kernels_fused < 1 && (opts().monopole_device_kernel_type != OFF || opts().multipole_device_kernel_type != OFF ||
										 opts().hydro_device_kernel_type != OFF)) {
		std::cerr << std::endl << "ERROR: ";
	...
	if (opts().monopole_device_kernel_type != OFF && opts().monopole_host_kernel_type != DEVICE_ONLY ||
		...
		std::cerr
			<< " - This build configuration only supports either the host or the device kernel active, not both of them at the same time!"
			<< std::endl
