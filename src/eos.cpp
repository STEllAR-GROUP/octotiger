...
real poly_K(real rho0, real mu) {
	...
	if (opts().radiation) {
		return POWER(rho0, -1.0 / 3.0)
				* ((c.kb * T0) / (mu * c.mh) + (4.0 * c.sigma * POWER(T0, 4.0)) / (3.0 * rho0 * c.c));
	} else {
		return POWER(rho0, -1.0 / 3.0) * ((c.kb * T0) *INVERSE (mu * c.mh));
	}
}
...
real struct_eos::energy(real d) const {
	...
	const real T = T0 * POWER(d * INVERSE(d0()), 1.0 / 3.0);
...
real struct_eos::enthalpy_to_density(real h) const {
	if (opts().eos == WD) {
		const real eps = poly_K(d0(), 4.0 / 3.0) * INVERSE (2.0 * A);
		const real b = B();
		const real f0 = (h * b) * INVERSE(8 * A);
		real x;
		real c0 = 1.0e-3;
		if (eps < f0 * c0) {
			x = (SQRT(2.0 * f0 + f0 * f0) - eps * (1.0 + f0)) * INVERSE (1.0 - eps * eps);
		} else if (f0 < eps * c0) {
			x = (2.0 * f0 + f0 * f0) * INVERSE (2.0 * eps * (1.0 - eps * eps));
		} else {
			x = (SQRT(eps * eps + 2 * f0 + f0 * f0) - eps * (1.0 + f0)) * INVERSE (1.0 - eps * eps);
		}
...
real struct_eos::density_to_enthalpy(real d) const {
	if (opts().eos == WD) {
		...
		real h;
		if (x > 0.01) {
			h = ((8.0 * A) * INVERSE( b )) * (SQRT(x * x + 1.0) + eps * x - 1.0);
		} else {
			h = (8.0 * A * INVERSE( b)) * (0.5 * x * x + eps * x);
		}
		return h;
	} else {
		if (d >= dC()) {
...
void struct_eos::set_d0_using_struct_eos(real newd, const struct_eos& other) {
	if (opts().eos == WD) {
		...
		if (!find_root(fff, min_h, max_h, new_h)) {
			stdout_printf("Error in struct_eos line %i\n", __LINE__);
			abort();
		}
	}
}
...
//		stdout_printf( "%e %e %e\n", icd, cm/m, core_frac);
	...
	if (!find_root(_func, 0.0, 1.0, interface_core_density, 1.0e-3)) {
		stdout_printf("UNable to produce core_Frac\n");
		abort();
	}
//	stdout_printf( "--------------- %e\n", s0());
