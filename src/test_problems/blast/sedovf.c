...
/* Subroutine */int sed_1d__(double *time, int *nstep, double * xpos,
	...
	slap_1.lstandard = FALSE_;
	slap_1.lsingular = FALSE_;
	slap_1.lvacuum = FALSE_;
	if ((d__1 = v2 - vstar, fabs(d__1)) <= 1e-4) {
		slap_1.lsingular = TRUE_;
	} else if (v2 < vstar - 1e-4) {
		slap_1.lstandard = TRUE_;
	} else if (v2 > vstar + 1e-4) {
		slap_1.lvacuum = TRUE_;
	}
	/* ..two apparent singularies, book's notation for omega2 and omega3 */
	slap_1.lomega2 = FALSE_;
	slap_1.lomega3 = FALSE_;
	if (fabs(denom2) <= 1e-4) {
		slap_1.lomega2 = TRUE_;
		denom2 = 1e-8f;
