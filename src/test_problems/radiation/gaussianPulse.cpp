...
static void initialize() {
	...
	c = physcon().c;
	centralRadiationEnergyDensity = convert.energyDensity(opts().radDiffEr0);
	timeOffset = convert.time(opts().radDiffT0);
	centralDensity = convert.massDensity(opts().radDiffRho0);
...
static Vector<Real, NDIM> fftWaveVector(int xi, int xj, int xk) {
	Vector<Real, NDIM> k;
	CgsToCode const convert;
...
static auto computeErHat(Real k, Real t) {
	FpeGuard fpeGuard{};
	Real const D = c / (3_R * extinctionCoefficient);
	Real const v = c * std::numbers::inv_sqrt3;
	Real const α = c * extinctionCoefficient * 0.5_R;
	Real const ErHat0 = centralRadiationEnergyDensity * std::pow(4_R * piR * D * timeOffset, 1.5_R);
	Real const k2 = k * k;
...
static auto computeEr(int xi, int yi, int zi, Real t) {
	...
	{
		bool doFFt;
		std::lock_guard<hpx::mutex> guard(mutex);
		doFFt = bool(t != ct);
			...
			for (int ki = 0; ki < N; ki++) {
				for (int kj = 0; kj < N; kj++) {
					for (int kk = 0; kk < N; kk++) {
						auto const k = fftWaveVector(ki, kj, kk);
						auto [E, dEdt] = computeErHat(abs(k), t);
...
std::vector<Real> analyticGaussianPulse(Real xPosition, Real yPosition, Real zPosition, Real t) {
	FpeGuard fpeGuard{};
	{
		static hpx::mutex mutex;
		std::lock_guard<hpx::mutex> guard(mutex);
	...
	auto const L = convert.length(2.0 * opts().xscale);
	xPosition = convert.length(xPosition);
	yPosition = convert.length(yPosition);
	zPosition = convert.length(zPosition);
	auto const N = OCTOTIGER_GRIDDIM << opts().max_level;
	auto const dx = L / N;
	size_t const xi = size_t(periodicWrap((xPosition + 0.5 * L) / dx, N));
	size_t const yi = size_t(periodicWrap((yPosition + 0.5 * L) / dx, N));
	size_t const zi = size_t(periodicWrap((zPosition + 0.5 * L) / dx, N));
