...
static void initialize() {
	...
	c = physcon().c;
	er0 = convert.energyDensity(opts().rad_diff_Er0);
	t0 = convert.time(opts().rad_diff_t0);
	rho0 = convert.massDensity(opts().rad_diff_rho0);
	ei0 = er0 * 1e-20;
	χ = opacity.extinction(rho0, 0_R, 1_R);
	printf("χ --------------> %e\n", χ);
}

static Real Eh(Real r) {
	Real const D = c / (3_R * χ);
	return er0 * std::exp(-0.25 * r * r / (D * t0));
}
...
static auto compute_ErHat(Real k, Real t) {
	...
	Real const α = c * χ * 0.5_R;
	Real const ErHat0 = er0 * std::pow(4_R * pi_R * D * t0, 1.5_R);
	Real const k2 = k * k;
	Real const disc = sqr(α) - sqr(v) * sqr(k);
	Real const e = std::exp(-(D * t0 * sqr(k) + α * t));
	complex E, dEdt;
	if (disc > 0_R) {
...
//	printf( "%e %e\n", r, sqrt(F2/E2));
	if (F2 > sqr(theta * c) * E2) {
		theta *= c * sqrt(E2 / F2);
		F *= theta;
	}
	ASSERT_NONNEGATIVE(E);
	G[spc_i] = G[rho_i] = rho0;
	G[tau_i] = pow(ei0, 1_R / gamma);
	G[egas_i] = ei0;
	R[0] = E;
	R[1] = F[0];
	R[2] = F[1];
