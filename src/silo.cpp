/*
 * silo.cpp
 *
 *  Created on: Oct 16, 2018
 *      Author: dmarce1
 */

#include "node_registry.hpp"
#include "silo.hpp"
#include "options.hpp"

HPX_PLAIN_ACTION(output_all, output_all_action);

#include <hpx/include/threads.hpp>
#include <hpx/include/run_as.hpp>

void output_all(std::string fname, int cycle) {
	std::vector<hpx::future<void>> futs;
	hpx::lcos::local::mutex mtx;
	std::string this_fname = fname + std::to_string(cycle) + std::to_string(hpx::get_locality_id()) + std::string(".silo");
	DBfile *db = DBCreateReal(this_fname.c_str(), DB_CLOBBER, DB_LOCAL, "Octo-tiger", DB_PDB);
	std::vector<const char*> mesh_names;
	int nfields = 0;
	if (opts.hydro) {
		nfields += 14;
	}
	if (opts.gravity) {
		nfields += 4;
	}
	std::vector<std::vector<char*>> field_names(nfields);
	auto optlist = DBMakeOptlist(1);
	int one = 1;
	DBAddOption(optlist, DBOPT_HIDE_FROM_GUI, &one);
	for (auto i = node_registry::begin(); i != node_registry::end(); i++) {
		//	auto this_ptr = i->second;
		//	auto loc = i->first;
		if (!i->second->refined()) {
			futs.push_back(
					hpx::async(
							[&mtx,&db,&mesh_names,&optlist,&field_names](node_location loc, node_server* this_ptr) {
								std::array<std::array<real, INX+1>, NDIM> X;
								const real dx = TWO / real(1 << loc.level()) / real(INX);
								for( int d = 0; d < NDIM; d++) {
									const real x0 = loc.x_location(d);
									for( int i = 0; i <= INX; i++) {
										X[d][i] = x0 + real(i) * dx;
									}
								}
								const std::string suffix = loc.to_str();
								const std::string mesh_name = std::string( "mesh_") + suffix;
								const char* mname = mesh_name.c_str();
								const char* coord_names[] = {"x", "y", "z"};
								const real* coords[] = {X[0].data(),X[1].data(),X[2].data()};
								constexpr int dims[] = {INX+1,INX+1,INX+1};
								constexpr int dims2[] = {INX,INX,INX};
								constexpr int data_type = DB_DOUBLE;
								constexpr int ndim = NDIM;
								constexpr int coord_type = DB_COLLINEAR;
								const auto scalar_output = this_ptr->get_hydro_grid().scalar_output_data(suffix);

								std::lock_guard<hpx::lcos::local::mutex> lock(mtx);

								DBPutQuadmesh( db, mname, coord_names, coords, dims, ndim, data_type, coord_type, optlist);
								char* ptr = new char[strlen(mname)+1];
								std::strcpy(ptr,mname);
								mesh_names.push_back(ptr);

								int f = 0;
								for( const auto& o : scalar_output) {
									DBPutQuadvar1(db, o.name_, mname, o.data_, dims2, ndim, (const void*) NULL, 0, DB_DOUBLE, DB_ZONECENT, optlist);
									char* ptr = new char[strlen(o.name_)+1];
									strcpy(ptr,o.name_);
									field_names[f].push_back(ptr);
									f++;
								}
								return;
							}, i->first, i->second));
		}
	}
	for (auto& f : futs) {
		f.get();
	}
	DBFreeOptlist(optlist);

	const int n_total_domains = mesh_names.size();
	static const std::vector<int> meshtypes(n_total_domains, DB_QUAD_RECT);
	static const std::vector<int> datatypes(n_total_domains, DB_QUADVAR);
	DBPutMultimesh(db, "mesh", n_total_domains, mesh_names.data(), meshtypes.data(), (decltype(optlist)) NULL);
	const auto top_field_names = grid::get_field_names();
	for (int f = 0; f < nfields; f++) {
		DBPutMultivar(db, top_field_names[f].c_str(), n_total_domains, field_names[f].data(), datatypes.data(), (decltype(optlist)) NULL);
	}
	for (auto ptr : mesh_names) {
		delete[] ptr;
	}
	for (int f = 0; f < nfields; f++) {
		for (auto& s : field_names[f]) {
			delete[] s;
		}
	}
	DBClose(db);
}

silo_var_t::silo_var_t(const std::string& name) {
	name_ = (char*) malloc(sizeof(char) * (name.size() + 1));
	data_ = (double*) malloc(sizeof(double) * INX * INX * INX);
	strcpy(name_, name.c_str());
}

double& silo_var_t::operator()(int i) {
	return (static_cast<double*>(data_))[i];

}

double silo_var_t::operator()(int i) const {
	return (static_cast<double*>(data_))[i];

}

silo_var_t::~silo_var_t() {
	if (name_ != NULL) {
		free(name_);
		free(data_);
	}
}

silo_var_t::silo_var_t(silo_var_t&& other) {
	name_ = other.name_;
	data_ = other.data_;
	other.name_ = NULL;
	other.data_ = NULL;
}
