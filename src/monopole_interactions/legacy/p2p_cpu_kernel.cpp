...
namespace fmm {
    namespace monopole_interactions {
        ...
        void p2p_cpu_kernel::cell_interactions(const cpu_monopole_buffer_t& __restrict__ mons,
            cpu_expansion_result_buffer_t& __restrict__ potential_expansions_SoA,
            ...
            for (int stencil_x = STENCIL_MIN; stencil_x <= STENCIL_MAX; stencil_x++) {
                ...
                for (int stencil_y = STENCIL_MIN; stencil_y <= STENCIL_MAX; stencil_y++) {
                    ...
                    for (int stencil_z = STENCIL_MIN; stencil_z <= STENCIL_MAX; stencil_z++) {
                        ...
                        const multiindex<> interaction_partner_index2(
                            cell_index.x + stencil_element.x, cell_index.y + stencil_element.y + 1,
                            cell_index.z + stencil_element.z);
                        ...
                        m2m_int_vector theta_c_rec_squared_int2 =
                            detail::distance_squared_reciprocal(
                        ...
                        const m2m_vector::mask_type mask2 =
                            theta_rec_squared > theta_c_rec_squared2;
                        if (Vc::none_of(mask) && Vc::none_of(mask2)) {
                            continue;
                        }
                        data_changed = true;
                        m2m_vector monopole;
                        Vc::where(mask, monopole) =
                        ...
                        const m2m_vector four[4] = {four_constants[index][0],
                            four_constants[index][1], four_constants[index][2],
                            four_constants[index][3]};

                        compute_monopole_interaction<m2m_vector>(
                            monopole, tmpstore1, four, d_components);
                        compute_monopole_interaction<m2m_vector>(
                            monopole2, tmpstore2, four, d_components);
                    }
                }
            }
