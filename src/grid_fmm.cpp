...
void load_multipole(taylor<4, T> &m, space_vector &c,
                    const gravity_boundary_type &data, integer iter,
                    bool monopole) {
  if (monopole) {
    m = T(0.0);
    m = T((*(data.m))[iter]);
...
#pragma GCC ivdep
          for (integer i = 0; i != simd_len; ++i) {
            const auto index =
                std::min(integer(li + i), integer(list_size - 1));
...
#pragma GCC ivdep
 ...
 #pragma GCC ivdep
    ...
    // Number of body-body interactions current leaf cell, probably includes interactions with bodies in neighboring cells  (David)
    const integer dsize = ilist_d.size();

    // Iterate interaction description between bodies in the current cell and closeby bodies (David)
  ...
  hpx::parallel::for_loop(
      ...
      [&mpoles, &com0, &ilist_n_bnd, type, this, &M](std::size_t si) {
        ...
        std::array<simd_vector, NDIM> simdY = {
            simd_vector(real(Y[0])), simd_vector(real(Y[1])),
            simd_vector(real(Y[2])),
        };
...
void grid::compute_boundary_interactions_multipole_monopole(
    gsolve_type type, const std::vector<boundary_interaction_type> &ilist_n_bnd,
    const gravity_boundary_type &mpoles) {
  PROF_BEGIN;
  auto &M = *M_ptr;
...
void grid::compute_boundary_interactions_monopole_multipole(
    gsolve_type type, const std::vector<boundary_interaction_type> &ilist_n_bnd,
    const gravity_boundary_type &mpoles) {
  PROF_BEGIN;
  auto &M = *M_ptr;
  ...
  std::array<real, NDIM> Xbase = {X[0][hindex(H_BW, H_BW, H_BW)],
...
void grid::compute_boundary_interactions_monopole_monopole(
    gsolve_type type, const std::vector<boundary_interaction_type> &ilist_n_bnd,
    const gravity_boundary_type &mpoles) {
  PROF_BEGIN;
  auto &M = *M_ptr;
...
void compute_ilist() {
  ...
  const auto theta = [](integer i0, integer j0, integer k0, integer i1,
                        integer j1, integer k1) {
...
expansion_pass_type
grid::compute_expansions(gsolve_type type,
...
multipole_pass_type
grid::compute_multipoles(gsolve_type type,
  ...
  for (integer inx = INX; (inx >= INX / 2); inx >>= 1) {
    ...
    auto child_index = [=](integer ip, integer jp, integer kp,
                           integer ci) -> integer {
