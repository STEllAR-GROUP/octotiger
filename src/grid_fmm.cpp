...
static std::vector<std::vector<boundary_interaction_type>> ilist_d_bnd(
		geo::direction::count());
static std::vector<std::vector<boundary_interaction_type>> ilist_n_bnd(
		geo::direction::count());
static taylor<4, real> factor;
extern options opts;

template<class T>
void load_multipole(taylor<4, T>& m, space_vector& c,
			...
			for (integer k = 0; k != G_NX; ++k) {
				...
				for (integer n = 0; n != NDIM; ++n) {
					...
					for (integer m = 0; m != NDIM; ++m) {
						...
						quad_moment[n][m] += mass * com[0][iii1][n]
								* com[0][iii1][m];
					}
				}
			}
...
void grid::compute_interactions(gsolve_type type) {
	...
	if (!is_leaf) {
		...
		for (integer li = 0; li < list_size; li += simd_len) {
			for (integer i = 0; i != simd_len && li + i < list_size; ++i) {
				...
				for (integer j = 10; j != 20; ++j) {
					if (type == RHO) {
						n0.ptr()[j][i] = M[iii1].ptr()[j]
								- M[iii0].ptr()[j] * (M[iii1]() / M[iii0]());
						n1.ptr()[j][i] = M[iii0].ptr()[j]
								- M[iii1].ptr()[j] * (M[iii0]() / M[iii1]());
					} else {
						n0.ptr()[j][i] = ZERO;
						n1.ptr()[j][i] = ZERO;
					}
		...
		const v4sd d0 = { 1.0 / dx, +1.0 / (dx * dx), +1.0 / (dx * dx), +1.0
				/ (dx * dx) };
		const v4sd d1 = { 1.0 / dx, -1.0 / (dx * dx), -1.0 / (dx * dx), -1.0
				/ (dx * dx) };
...
void grid::compute_boundary_interactions(gsolve_type type,
		const geo::direction& dir, bool is_monopole,
		const gravity_boundary_type& mpoles) {
	if (!is_leaf) {
		if (!is_monopole) {
			compute_boundary_interactions_multipole_multipole(type,
					ilist_n_bnd[dir], mpoles);
		} else {
			compute_boundary_interactions_monopole_multipole(type,
					ilist_d_bnd[dir], mpoles);
		}
	} else {
		if (!is_monopole) {
			compute_boundary_interactions_multipole_monopole(type,
					ilist_d_bnd[dir], mpoles);
		} else {
			compute_boundary_interactions_monopole_monopole(type,
					ilist_d_bnd[dir], mpoles);
		}
	}
	...
	for (integer si = 0; si != ilist_n_bnd.size(); ++si) {
		...
		for (integer li = 0; li < list_size; li += simd_len) {
			for (integer i = 0; i != simd_len && li + i < list_size; ++i) {
				...
				for (integer j = 10; j != 20; ++j) {
					if (type == RHO) {
						n0.ptr()[j][i] = m0.ptr()[j][i]
								- M[iii0].ptr()[j] * (m0()[i] / M[iii0]());
					} else {
						n0.ptr()[j][i] = ZERO;
					}
				}
			...
			std::array<simd_vector, NDIM> B0 = { simd_vector(0.0), simd_vector(
					0.0), simd_vector(0.0) };
...
void grid::compute_boundary_interactions_monopole_multipole(gsolve_type type,
	...
	for (integer si = 0; si != ilist_n_bnd.size(); ++si) {
		...
		const integer iii1 = ilist_n_bnd[si].second;
		for (integer d = 0; d != NDIM; ++d) {
			Y[d] = ilist_n_bnd[si].x[d] * dx
					+ this->X[d][hindex(H_BW,H_BW,H_BW)];
		}
		m0 = (*(mpoles.m))[index];
		for (integer li = 0; li < list_size; li += simd_len) {
			for (integer i = 0; i != simd_len && li + i < list_size; ++i) {
...
void compute_ilist() {
	...
	std::array < std::vector<interaction_type>, geo::direction::count()
			> ilist_n0_bnd;
	std::array < std::vector<interaction_type>, geo::direction::count()
			> ilist_d0_bnd;
	const real theta0 = opts.theta;
	const auto theta =
			[](integer i0, integer j0, integer k0, integer i1, integer j1, integer k1) {
				...
				for (integer i1 = ilb; i1 < iub; ++i1) {
					for (integer j1 = jlb; j1 < jub; ++j1) {
						for (integer k1 = klb; k1 < kub; ++k1) {
							...
							const real theta_c = theta(i0_c, j0_c, k0_c, i1_c,
									j1_c, k1_c);
							...
							if (theta_c > theta0 && theta_f <= theta0) {
								...
								np.x[YDIM] = j1;
								np.x[ZDIM] = k1;
								if (interior(i1, j1, k1)
										&& interior(i0, j0, k0)) {
									if (iii1 > iii0) {
										ilist_n0.push_back(np);
									}
								} else if (interior(i0, j0, k0)) {
									ilist_n0_bnd[neighbor_dir(i1, j1, k1)].push_back(
											np);
								}
							}
							if (theta_c > theta0) {
								...
								if (interior(i1, j1, k1)
									...
									ilist_d0_bnd[neighbor_dir(i1, j1, k1)].push_back(
											dp);
								}
							}
							if (theta_f <= theta0) {
	...
	printf("# direct = %i\n", int(max_d));
	ilist_n = std::vector < interaction_type
			> (ilist_n0.begin(), ilist_n0.end());
	ilist_d = std::vector < interaction_type
			> (ilist_d0.begin(), ilist_d0.end());
	ilist_r = std::vector < interaction_type
			> (ilist_r0.begin(), ilist_r0.end());
...
expansion_pass_type grid::compute_expansions(gsolve_type type,
		const expansion_pass_type* parent_expansions) {
	PROF_BEGIN;
	...
	for (integer ip = 0; ip != nxp; ++ip) {
		for (integer jp = 0; jp != nxp; ++jp) {
			for (integer kp = 0; kp != nxp; ++kp) {
				...
				if (!is_root) {
					const integer index = (INX * INX / 4) * (ip)
							+ (INX / 2) * (jp) + (kp);
					for (integer j = 0; j != 20; ++j) {
...
multipole_pass_type grid::compute_multipoles(gsolve_type type,
		const multipole_pass_type* child_poles) {
	PROF_BEGIN;
	integer lev = 0;
	...
	if (type == RHO) {
		...
		const std::array<real, NDIM> x0 = { X[XDIM][iii0], X[YDIM][iii0],
				X[ZDIM][iii0] };
	...
	for (integer inx = INX; (inx >= INX / 2); inx >>= 1) {
		...
		auto child_index =
				[=](integer ip, integer jp, integer kp, integer ci) -> integer {
				...
				for (integer kp = 0; kp != nxp; ++kp) {
					...
					if (lev != 0) {
						...
						if (child_poles == nullptr) {
							const integer iiih =
									hindex(ip + H_BW , jp + H_BW , kp + H_BW );
...
gravity_boundary_type grid::get_gravity_boundary(const geo::direction& dir,
		bool is_local) {
	PROF_BEGIN;
	...
	if (!is_local) {
		data.allocate();
		integer iter = 0;
		const std::vector<boundary_interaction_type>& list =
				ilist_n_bnd[dir.flip()];
		if (is_leaf) {
		...
		if (is_leaf) {
			...
			data.x = std::shared_ptr < std::vector
					< space_vector >> (&com[0], nuldel);
		}
	}
	PROF_END;
	return data;
