...
void node_server::register_counters() {
	hpx::performance_counters::install_counter_type("/octotiger/subgrids", &cumulative_nodes_count,
			"total number of subgrids processed");
	hpx::performance_counters::install_counter_type("/octotiger/subgrid_leaves", &cumulative_leafs_count,
			"total number of subgrid leaves processed");
	hpx::performance_counters::install_counter_type("/octotiger/amr_bounds", &cumulative_amrs_count,
			"total number of amr bounds processed");
}
...
future<void> node_server::exchange_flux_corrections() {
	...
	return hpx::when_all(std::move(futs)).then(hpx::util::annotated_function([](future<decltype(futs)> fout) {
		auto fin = GET(fout);
		for (auto &f : fin) {
			GET(f);
		}
...
node_server::node_server(const node_location &_my_location, integer _step_num, bool _is_refined, real _current_time,
		...
		const std::vector<hpx::id_type> &_c, std::size_t _hcycle, std::size_t _rcycle, std::size_t _gcycle,
		integer position_) {
...
void node_server::compute_fmm(gsolve_type type, bool energy_account, bool aonly) {
	...
	for (auto const &dir : geo::direction::full_set()) {
		if (!neighbors[dir].empty()) {
			if (!neighbor_signals[dir].try_acquire_for(std::chrono::duration<int>(100))) {
...
//             const auto gid = neighbors[dir].get_gid();
	...
	for (geo::direction const &dir : geo::direction::full_set()) {
		if (!neighbors[dir].empty()) {
			all_neighbor_interaction_data.push_back(GET(neighbor_gravity_channels[dir].get_future(gcycle)));
	...
	if (!grid_ptr->get_leaf()) {
		...
		octotiger::fmm::multipole_interactions::multipole_kernel_interface(mon_ptr, M_ptr, com_ptr,
				all_neighbor_interaction_data, type, grid_ptr->get_dx(), is_direction_empty, Xbase, grid_ptr,
				grid_ptr->get_root());
	} else { // ... we are a monopole
		octotiger::fmm::monopole_interactions::monopole_kernel_interface(mon_ptr, com_ptr, all_neighbor_interaction_data,
				type, grid_ptr->get_dx(), is_direction_empty, grid_ptr, contains_multipole);
	}
