	...
	if (is_refined) {
		for (integer ci = 0; ci != NCHILD; ++ci) {
			...
			for (integer d = 0; d != NDIM; ++d) {
				...
				child_sibs_f[thisf] = std::make_shared<hpx::future < hpx::id_type >>(hpx::make_ready_future(children[flip].get_gid()));
		...
		for( auto i = cfuts.begin(); i != cfuts.end(); ++i) {
			GET(*i);
		}
	}
