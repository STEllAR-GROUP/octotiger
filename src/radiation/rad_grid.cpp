...
void rad_grid::rad_imp(std::vector<real>& egas, std::vector<real>& tau, std::vector<real>& sx,
    ...
    //
    //                /*	if (((U[fx_i][iiir]) * (U[fx_i][iiir]) + (U[fy_i][iiir]) *
    //                (U[fy_i][iiir])
    //                 + (U[fz_i][iiir]) * (U[fz_i][iiir])) / (U[er_i][iiir] * U[er_i][iiir] *
    //                 physcon().c
...
void node_server::compute_radiation(real dt, real omega) {
    ...
    const real clight = physcon().c / opts().clight_retard;
    const real max_dt = min_dx / clight / 2;
...
void rad_grid::compute_flux(real omega) {
    auto const minmod_theta = [](Real a, Real b) {
        auto const minmod = [](Real a, Real b) {
            return (std::copysign(0.5_R, a) + std::copysign(0.5_R, b)) *
    ...
    for (auto dir = 0; dir < NDIM; dir++) {
        ...
        for (auto i = dn; i < RAD_N3 - dn; i++) {
            ...
            for (auto d = 0; d < NDIM; d++) {
                auto const dbeta =
                    minmod_theta(beta[i + dn][d] - beta[i][d], beta[i][d] - beta[i - dn][d]);
        ...
        for (auto i = 2 * dn; i < RAD_N3 - dn; i++) {
            ...
            auto const fE =
                (lambda_r * fEl - lambda_l * fEr + lambda_r * lambda_l * (Er - El)) * ilambda;
            auto const fF =
                (lambda_r * fFl - lambda_l * fFr + lambda_r * lambda_l * (Fr - Fl)) * ilambda;
