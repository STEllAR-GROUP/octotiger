...
hpx::future<real> node_client::scf_update(real com, real omega, real c1, real c2, real c1_x, real c2_x, real l1_x,
		struct_eos e1, struct_eos e2) const {
		...
		for (integer j = H_BW; j != H_NX - H_BW; ++j) {
			for (integer k = H_BW; k != H_NX - H_BW; ++k) {
				...
				if (g <= 0.0) {
					...
					ASSERT_NONAN(C);
					new_rho = std::max(this_struct_eos.enthalpy_to_density(std::max(C - phi_eff, 0.0)),
					rho_floor);
				} else {
					new_rho = rho_floor;
				}
...
//		profiler_output(stdout);
		...
		real f0 = scf_options::M1 / (diags.m[AINDEX]);
		real f1 = scf_options::M2 / (diags.m[DINDEX]);
...
//		f = (f + 1.0)/2.0;
		...
		real is1 = diags.z_moment[AINDEX];
		real is2 = diags.z_moment[DINDEX];
		iorb -= is1 + is2;
		real M1 = diags.m[AINDEX];
		real M2 = diags.m[DINDEX];
		...
		c1_x = diags.com[AINDEX][XDIM];
		c2_x = diags.com[DINDEX][XDIM];
		rho1 = diags.rho_max[AINDEX];
		rho2 = diags.rho_max[DINDEX];
		...
		if (diags.virial_norm == 0.0) {
			printf("ZERO              !!!!\n");
			abort();
		}
		const real virial = diags.virial / diags.virial_norm;
		const real v1 = diags.stellar_vol[AINDEX];
		const real v2 = diags.stellar_vol[DINDEX];
		...
		if (i % 5 == 0) {
			printf(
					...
					"rho1", "rho2", "M1", "M2", "omega", "virial", "core_frac_1", "core_frac_2", "jorb", "jmin", "amin", "jtot",
					"com", "spin_ratio", "r1", "r2", "iorb", "pvol", "proche", "svol", "sroche");
		}
		lprintf((opts.data_dir + "log.txt").c_str(),
				"%i %13e %13e %13e %13e %13e %13e %13e %13e %13e %13e %13e %13e %13e %13e %13e %13e %13e %13e %13e %13e %13e\n",
				i, rho1, rho2, M1, M2, omega, virial, core_frac_1, core_frac_2, jorb, jmin, amin, j1 + j2 + jorb, com,
				spin_ratio, r1, r2, iorb, diags.stellar_vol[AINDEX], diags.roche_vol[AINDEX], diags.stellar_vol[DINDEX],
				diags.roche_vol[DINDEX]);
		if (i % 10 == 0) {
