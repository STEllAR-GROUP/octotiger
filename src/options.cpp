...
bool options::process_options(int argc, char* argv[]) {
    ...
    command_opts.add_options()    //
        ...
        ("sod_pr", po::value<real>(&(opts().sod_pr))->default_value(0.1),
            "pressure in the right part of the grid")    //
        ...
        ("star_alpha", po::value<real>(&(opts().star_alpha))->default_value(1.0 / (3.0 * 3.65375)),
            "scaling factor for the Lane-Emden equation")    // for default n=3/2, ksi_1=3.65375 and
                                                             // alpha=rmax/ksi_1
        ...
        ("star_rho_out", po::value<real>(&(opts().star_rho_out))->default_value(1.0e-10),
            "density outside the star")    //
        ("star_egas_out", po::value<real>(&(opts().star_egas_out))->default_value(1.0e-10),
            "gas energy outside the star")    //
        ("moving_star_xvelocity",
            po::value<real>(&(opts().moving_star_xvelocity))->default_value(1.0),
            "velocity of the star in the x-direction")    //
        ("moving_star_yvelocity",
            po::value<real>(&(opts().moving_star_yvelocity))->default_value(1.0),
            "velocity of the star in the y-direction")    //
        ("moving_star_zvelocity",
            po::value<real>(&(opts().moving_star_zvelocity))->default_value(1.0),
            "velocity of the star in the z-direction")    //
        ("clight_retard", po::value<real>(&(opts().clight_retard))->default_value(1.0),
            "retardation factor for speed of light")    //
        ("driving_rate", po::value<real>(&(opts().driving_rate))->default_value(0.0),
            "angular momentum loss driving rate")    //
        ("driving_time", po::value<real>(&(opts().driving_time))->default_value(0.0),
            "A.M. driving rate time")    //
        ("entropy_driving_time",
            po::value<real>(&(opts().entropy_driving_time))->default_value(0.0),
            "entropy driving rate time")    //
        ("entropy_driving_rate",
            po::value<real>(&(opts().entropy_driving_rate))->default_value(0.0),
        ...
        ("amrbnd_order", po::value<integer>(&(opts().amrbnd_order))->default_value(1),
            "amr boundary interpolation order")    //
        ("scf_output_frequency",
            po::value<integer>(&(opts().scf_output_frequency))->default_value(25),
            "Frequency of SCF output")    //
        ("silo_num_groups", po::value<integer>(&(opts().silo_num_groups))->default_value(-1),
            "Number of SILO I/O groups")    //
        ("core_refine", po::value<bool>(&(opts().core_refine))->default_value(false),
            "refine cores by one more level")    //
        ...
        ("radiation", po::value<bool>(&(opts().radiation))->default_value(false),
            "radiation on/off")    //
        ("correct_am_hydro", po::value<bool>(&(opts().correct_am_hydro))->default_value(true),
        ...
        ("cdisc_detect", po::value<bool>(&(opts().cdisc_detect))->default_value(true),
            "PPM contact discontinuity detection")    //
        ("disable_output", po::value<bool>(&(opts().disable_output))->default_value(false),
            "disable silo output")    //
        ("disable_diagnostics",
            po::value<bool>(&(opts().disable_diagnostics))->default_value(false),
            "disable diagnostics")    //
        ...
        ("restart_filename", po::value<std::string>(&(opts().restart_filename))->default_value(""),
            "restart filename")    //
        ("stop_time",
            po::value<real>(&(opts().stop_time))->default_value(std::numeric_limits<real>::max()),
            "time to end simulation")    //
        ("stop_step",
            po::value<integer>(&(opts().stop_step))
                ->default_value(std::numeric_limits<integer>::max() - 1),
        ...
        ("amr_boundary_kernel_type",
            po::value<amr_boundary_type>(&(opts().amr_boundary_kernel_type))
                ->default_value(AMR_OPTIMIZED),
...
#endif
        ("cuda_number_gpus",
            po::value<size_t>(&(opts().cuda_number_gpus))->default_value(size_t(0)),
            "cuda streams per HPX locality")    //
        ("cuda_streams_per_gpu",
            po::value<size_t>(&(opts().cuda_streams_per_gpu))->default_value(size_t(0)),
            "cuda streams per GPU (per locality)")    //
        ("cuda_buffer_capacity",
            po::value<size_t>(&(opts().cuda_buffer_capacity))->default_value(size_t(5)),
    ...
    if (opts().theta < octotiger::fmm::THETA_FLOOR) {
        std::cerr << "theta " << theta
                  << " is too small since Octo-Tiger was compiled for a minimum of "
...
#define SHOW(opt) std::cout << std::string(#opt) << " = " << to_string(opt) << '\n';
        ...
        if (silo_num_groups > num_loc) {
            print("Number of SILO file groups cannot be greater than number of localities. Setting "
                  "silo_num_groupds to %li\n",
                num_loc);
    ...
    if (opts().gravity) {
        if (opts().multipole_device_kernel_type == interaction_device_kernel_type::CUDA &&
            ...
            std::cerr
                << "Due to a current implementation limitation in the load balancing, "
                << " multipole cuda device kernels cannot be mixed with the respective kokkos host "
                   "kernel!"
                << std::endl
        ...
        if (opts().monopole_device_kernel_type == interaction_device_kernel_type::CUDA &&
            ...
            std::cerr
                << "Due to a current implementation limitation in the load balancing, "
                << " monopole cuda device kernels cannot be mixed with the respective kokkos host "
                   "kernel!"
                << std::endl
                << " Please choose a different host kernel "
                << "(or move to kokkos device kernel with "
