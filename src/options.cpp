...
bool options::process_options(int argc, char *argv[]) {
	...
	("second_solid_sphere_radius", po::value<real>(&(opts().second_solid_sphere_radius))->default_value(0.01 / 3), "radius of a second sphere in the double solid sphere problem")     //
	...
	("relaxation_period", po::value<real>(&(opts().damping_period))->default_value(0.0), "relaxing the star by damping the velocities, usually set to be the star's free-fall time")      //
