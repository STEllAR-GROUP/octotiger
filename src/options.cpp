...
void do_options(options& opts) {

	namespace po = boost::program_options;

	po::options_description command_opts("options");
	command_opts.add_options()("xscale", po::value<real>(&(opts.xscale))->default_value(1.0), "grid scale")("omega",
			po::value<real>(&(opts.omega))->default_value(0.0), "(initial) angular frequency")("variable_omega",
			po::value<bool>(&(opts.vomega))->default_value(false), "use variable omega")("driving_rate",
			po::value<real>(&(opts.driving_rate))->default_value(0.0), "angular momentum loss driving rate")("driving_time",
			po::value<real>(&(opts.driving_time))->default_value(0.0), "A.M. driving rate time")("entropy_driving_rate",
			po::value<real>(&(opts.driving_rate))->default_value(0.0), "entropy loss driving rate")("entropy_driving_time",
			po::value<real>(&(opts.driving_time))->default_value(0.0), "entropy driving rate time")("core_refine",
			po::value<bool>(&(opts.core_refine))->default_value(false), "refine cores by one more level")("accretor_refine",
			po::value<integer>(&(opts.accretor_refine))->default_value(0), "number of extra levels for accretor")(
			"donor_refine", po::value<integer>(&(opts.donor_refine))->default_value(0), "number of extra levels for donor")(
			"ngrids", po::value<integer>(&(opts.ngrids))->default_value(-1), "fix numbger of grids")("refinement_floor",
			po::value<real>(&(opts.refinement_floor))->default_value(-1.0), "density refinement floor")("theta",
			po::value<real>(&(opts.theta))->default_value(0.0),
			"controls nearness determination for FMM, must be between 1/3 and 1/2")("eos",
			po::value<eos_type>(&(opts.eos))->default_value(IDEAL), "gas equation of state");
}
...
void options::show_help() {
	printf(
			"Command line options for Octo-tiger\n\n"
...
bool options::process_options(int argc, char* argv[]) {
	...
	donor_refine = 0;
	accretor_refine = 0;
	std::cout << M2M_KERNEL_OPT << std::endl << P2P_KERNEL_OPT << std::endl << P2M_KERNEL_OPT << std::endl;
