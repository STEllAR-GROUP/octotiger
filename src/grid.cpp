...
char const* grid::field_names[] = {"rho", "egas", "sx", "sy", "sz", "tau", "pot", "zx", "zy", "zz",
    "primary_core", "primary_envelope", "secondary_core", "secondary_envelope", "vacuum", "phi",
    "gx", "gy", "gz", "vx", "vy", "vz", "eint", "zzs"};
...
void grid::set_hydro_boundary(
    const std::vector<real>& data, const geo::direction& dir, integer width, bool etot_only) {
    PROF_BEGIN;
...
std::vector<real> grid::get_hydro_boundary(
...
inline real minmod(real a, real b) {
    //    return (std::copysign(HALF, a) + std::copysign(HALF, b)) * std::min(std::abs(a),
    //    std::abs(b));
    bool a_is_neg = a < 0;
    bool b_is_neg = b < 0;
...
std::vector<real> grid::get_prolong(
    const std::array<integer, NDIM>& lb, const std::array<integer, NDIM>& ub, bool etot_only) {
    PROF_BEGIN;
    auto& dUdx = TLS_dUdx();
    auto& tmpz = TLS_zz();
    std::vector<real> data;
...
std::vector<real> grid::get_restrict() const {
    PROF_BEGIN;
    constexpr integer Size = NF * INX * INX * INX / NCHILD + NF;
    std::vector<real> data;
    data.reserve(Size);
    for (integer field = 0; field != NF; ++field) {
...
void grid::compute_primitives(
    const std::array<integer, NDIM> lb, const std::array<integer, NDIM> ub, bool etot_only) const {
    PROF_BEGIN;
    auto& V = TLS_V();
    if (!etot_only) {
...
void grid::compute_primitive_slopes(real theta, const std::array<integer, NDIM> lb,
...
#pragma GCC ivdep
                for (integer k = lb[ZDIM]; k != ub[ZDIM]; ++k) {
                    ...
                    const auto v0 = v[iii];
                    dVdx[XDIM][f][iii] =
                        minmod_theta(v[iii + H_DNX] - v0, v0 - v[iii - H_DNX], theta);
                    dVdx[YDIM][f][iii] =
                        minmod_theta(v[iii + H_DNY] - v0, v0 - v[iii - H_DNY], theta);
                    dVdx[ZDIM][f][iii] =
                        minmod_theta(v[iii + H_DNZ] - v0, v0 - v[iii - H_DNZ], theta);
                }
            }
        }
    }
            ...
            for (integer k = lb[ZDIM]; k != ub[ZDIM]; ++k) {
                ...
                for (integer d0 = 0; d0 != NDIM; ++d0) {
                    for (integer d1 = 0; d1 != NDIM; ++d1) {
                        dV_sym[d1][d0] =
                            (dVdx[d0][sx_i + d1][iii] + dVdx[d1][sx_i + d0][iii]) / 2.0;
                        dV_ant[d1][d0] = 0.0;
                    }
                }
                //		if( opts.ang_con) {
                ...
                for (integer d0 = 0; d0 != NDIM; ++d0) {
                    for (integer d1 = 0; d1 != NDIM; ++d1) {
                        ...
                        dVdx[d0][sx_i + d1][iii] =
                            minmod(tmp, 2.0 / theta * dVdx[d0][sx_i + d1][iii]);
                    }
                }
            }
        }
...
void grid::compute_conserved_slopes(
    const std::array<integer, NDIM> lb, const std::array<integer, NDIM> ub, bool etot_only) {
    PROF_BEGIN;
    auto& dVdx = TLS_dVdx();
    auto& dUdx = TLS_dUdx();
...
#pragma GCC ivdep
                    for (integer k = lb[ZDIM]; k != ub[ZDIM]; ++k) {
                        ...
                        if (node_server::is_gravity_on()) {
                            dU[pot_i][iii] =
                                V[pot_i][iii] * dV[rho_i][iii] + dV[pot_i][iii] * V[rho_i][iii];
                        }
                        dU[egas_i][iii] =
                            V[egas_i][iii] * dV[rho_i][iii] + dV[egas_i][iii] * V[rho_i][iii];
                        for (integer d1 = 0; d1 != NDIM; ++d1) {
                            dU[sx_i + d1][iii] = V[sx_i + d1][iii] * dV[rho_i][iii] +
                                dV[sx_i + d1][iii] * V[rho_i][iii];
                            dU[egas_i][iii] +=
                                V[rho_i][iii] * (V[sx_i + d1][iii] * dV[sx_i + d1][iii]);
...
inline void limit_slope(real& ql, real q0, real& qr) {
    ...
    real sqr_tmp1_6 = sqr(tmp1) / 6.0;
    real tmp1_q0 =tmp1 * (q0 - 0.5 * tmp2);

    if (tmp1_q0 > sqr_tmp1_6) {
        ql = 3.0 * q0 - 2.0 * qr;
    } else if (-sqr_tmp1_6 > tmp1_q0) {
        qr = 3.0 * q0 - 2.0 * ql;
    }
};
...
#pragma GCC ivdep
        for (integer iii = H_NX * H_NX; iii != H_N3 - H_NX * H_NX; ++iii) {
            if (field == 1) {
                //	printf("%i %i %i\n", int(iii / (H_NX * H_NX)), int((iii / H_NX) % H_NX), int(iii
                //% H_NX));
                //	printf("%e %e %e\n", X[XDIM][iii], X[YDIM][iii], X[ZDIM][iii]);
                //	printf("%e %e %e\n", V[field][iii + H_DNX], V[field][iii], V[field][iii -
                //H_DNX]);
                //	printf("%e %e %e\n", V[field][iii + H_DNY], V[field][iii], V[field][iii -
                //H_DNY]);
                //	printf("%e %e %e\n", V[field][iii + H_DNZ], V[field][iii], V[field][iii -
                //H_DNZ]);
                //	printf("%e %e %e\n", V[rho_i][iii + H_DNX], V[rho_i][iii], V[rho_i][iii -
                //H_DNX]);
                // printf("%e %e %e\n", V[rho_i][iii + H_DNY], V[rho_i][iii], V[rho_i][iii -
                // H_DNY]);
                //	printf("%e %e %e\n", V[rho_i][iii + H_DNZ], V[rho_i][iii], V[rho_i][iii -
                //H_DNZ]);
            }
            slpx[field][iii] = minmod_theta(
                Vfield[iii + H_DNX] - Vfield[iii], Vfield[iii] - Vfield[iii - H_DNX], theta_x);
            slpy[field][iii] = minmod_theta(
                Vfield[iii + H_DNY] - Vfield[iii], Vfield[iii] - Vfield[iii - H_DNY], theta_y);
            slpz[field][iii] = minmod_theta(
                Vfield[iii + H_DNZ] - Vfield[iii], Vfield[iii] - Vfield[iii - H_DNZ], theta_z);
        }
    }
...
#pragma GCC ivdep
            for (integer iii = 0; iii != H_N3 - H_NX * H_NX; ++iii) {
                UfFXPfield[iii] = UfFXMfield[iii + H_DNX] =
                    average(Vfield[iii + H_DNX], Vfield[iii]);
            }
#pragma GCC ivdep
            for (integer iii = H_NX * H_NX; iii != H_N3 - H_NX * H_NX; ++iii) {
...
#pragma GCC ivdep
            for (integer iii = H_NX * H_NX; iii != H_N3 - H_NX * H_NX; ++iii) {
                ...
                UfFXPfield[iii] = u0 + HALF * slpxfield[iii];
                UfFXMfield[iii] = u0 - HALF * slpxfield[iii];
            }
        }
...
#pragma GCC ivdep
            for (integer iii = 0; iii != H_N3 - H_NX * H_NX; ++iii) {
                UfFYPfield[iii] = UfFYMfield[iii + H_DNY] =
                    average(Vfield[iii + H_DNY], Vfield[iii]);
            }
#pragma GCC ivdep
            for (integer iii = H_NX * H_NX; iii != H_N3 - H_NX * H_NX; ++iii) {
...
#pragma GCC ivdep
            for (integer iii = 0; iii != H_N3 - H_NX * H_NX; ++iii) {
                UfFZPfield[iii] = UfFZMfield[iii + H_DNZ] =
                    average(Vfield[iii + H_DNZ], Vfield[iii]);
            }
#pragma GCC ivdep
            for (integer iii = H_NX * H_NX; iii != H_N3 - H_NX * H_NX; ++iii) {
...
#pragma GCC ivdep
            for (integer iii = H_NX * H_NX; iii != H_N3 - H_NX * H_NX; ++iii) {
                const real& u0 = Vfield[iii];
                UfFZPfield[iii] = u0 + HALF * slpzfield[iii];
                UfFZMfield[iii] = u0 - HALF * slpzfield[iii];
            }
        }
    }
...
#pragma GCC ivdep
    ...
    if (node_server::is_gravity_on()) {
        ...
        for (integer iii = H_NX * H_NX; iii != H_N3 - H_NX * H_NX; ++iii) {
            inplace_average(UfFXMpot_i[iii], UfFXPpot_i[iii - H_DNX]);
            inplace_average(UfFYMpot_i[iii], UfFYPpot_i[iii - H_DNY]);
            inplace_average(UfFZMpot_i[iii], UfFZPpot_i[iii - H_DNZ]);
        }
    }
    for (integer field = 0; field != NF; ++field) {
    ...
    for (integer dim = 0; dim != NDIM; ++dim) {
        ...
        for (integer k = H_BW; k != H_NX - H_BW; ++k) {
            for (integer j = H_BW; j != H_NX - H_BW; ++j) {
                ...
                for (integer field = 0; field != NF; ++field) {
                    for (integer i = H_BW; i != H_NX - H_BW + 1; ++i) {
                        const integer i0 = F_DN[dx_i] * (i - H_BW) + F_DN[dy_i] * (j - H_BW) +
                            F_DN[dz_i] * (k - H_BW);
                    ...
                    for (integer i = ilb; i != iub; ++i) {
                        ...
                        if (field == sx_i + dim) {
                            ...
                            case OUTFLOW:
                                ...
                                if (this_rho != ZERO) {
                                    U[egas_i][iii] +=
                                        HALF * (after * after - before * before) / this_rho;
                                }
                                break;
                            }
                        } else {
...
#pragma GCC ivdep
            for (integer k = H_BW; k != H_NX - H_BW; ++k) {
                ...
                dUdt[zz_i][iii0] +=
                    omega * (X[XDIM][iii] * U[sx_i][iii] + X[YDIM][iii] * U[sy_i][iii]);
                //			}
            }
        }
    }
