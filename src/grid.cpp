...
std::vector<real> grid::get_hydro_boundary(const geo::direction& dir, integer width, bool tau_only) {
	...
	integer size;
	if (!tau_only) {
		size = NF * get_boundary_size(lb, ub, dir, INNER, H_NX, INX, width);
	} else {
		size = get_boundary_size(lb, ub, dir, INNER, H_NX, INX, width);
	}
	data.resize(size);
	integer iter = 0;
...
#pragma GCC ivdep
			for (integer k = H_BW; k != H_NX - H_BW; ++k) {
				...
				src[sx_i][iii0] += rho * G[gx_i][iiig];
				src[sy_i][iii0] += rho * G[gy_i][iiig];
				src[sz_i][iii0] += rho * G[gz_i][iiig];
				src[sx_i][iii0] += omega * U[sy_i][iii];
				src[sy_i][iii0] -= omega * U[sx_i][iii];
				src[egas_i][iii0] -= omega * X[YDIM][iii] * rho * G[gx_i][iiig];
				src[egas_i][iii0] += omega * X[XDIM][iii] * rho * G[gy_i][iiig];
#ifdef USE_DRIVING
