...
tar -xzf ~/Downloads/octotiger-radiation-results-cpp.tar.gz -C ~/workspace/octotiger
~/workspace/octotiger/test_results/build_cpp.sh release
```
...
The runner uses an already-configured Octo-TIGER build directory, such as
`~/workspace/octotiger/release`, and reads INX from its CMake cache. It rebuilds
...
```bash
cd ~
~/workspace/octotiger/test_results/results.sh install --check
~/workspace/octotiger/test_results/results.sh install
```
...
```bash
cd ~
~/workspace/octotiger/test_results/run_live.sh all 2 3 4 release --threads 12
```
...
```bash
cd ~
~/workspace/octotiger/test_results/run_live.sh wave 2 release --threads 12 --no-build
```
...
The runner reads your existing `test_results/configs/*.ini`, scales those
original templates to CGS, and writes each run's effective configuration as
...
Each batch prints its `index.html` path and normally opens it. `--no-open`
...
```bash
cd ~
BATCH=~/workspace/octotiger/test_results/results/live-20260917-084723-426280
~/workspace/octotiger/test_results/results.sh pages "$BATCH"
```
...
```bash
cd ~
~/workspace/octotiger/test_results/run_live.sh \
  --resume ~/workspace/octotiger/test_results/results/live-YYYYMMDD-HHMMSS-XXXXXX \
  --threads 12
```
...
```bash
cd ~
BATCH=~/workspace/octotiger/test_results/results/live-YYYYMMDD-HHMMSS-XXXXXX
~/workspace/octotiger/test_results/results.sh check "$BATCH"
~/workspace/octotiger/test_results/results.sh plot "$BATCH"
~/workspace/octotiger/test_results/results.sh movies "$BATCH" --seconds 20 --fps 30
```
...
```bash
~/workspace/octotiger/test_results/results.sh plot "$BATCH"
~/workspace/octotiger/test_results/results.sh movies "$BATCH"
```
...
cd ~
BATCH=~/workspace/octotiger/test_results/results/live-YYYYMMDD-HHMMSS-XXXXXX
~/workspace/octotiger/test_results/results.sh sessions "$BATCH"
```
...
```bash
cd ~
SESSION=~/workspace/octotiger/test_results/visit_sessions/NAME.session
...
```bash
~/workspace/octotiger/test_results/results.sh sessions "$BATCH" --view 3d
```
...
```bash
cd ~
cmake -S ~/workspace/octotiger/test_results \
  -B ~/workspace/octotiger/test_results/.build -DBUILD_TESTING=ON
cmake --build ~/workspace/octotiger/test_results/.build -j 4
ctest --test-dir ~/workspace/octotiger/test_results/.build --output-on-failure
```
...
```bash
cd ~
~/workspace/octotiger/test_results/results.sh validate-serial \
