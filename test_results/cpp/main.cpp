...
Options arguments(int argc, char **argv)
{
	...
	o.root = here.filename() == ".build" ? here.parent_path().parent_path() : fs::current_path();
	o.jobs = std::max(1u, std::min(8u, std::thread::hardware_concurrency()));
	int start = 1;
	if (argc > 1 &&
		std::set<std::string>{"live", "run", "plot", "pages", "movies", "install", "validate-serial", "check"}
			.contains(argv[1])) {
		o.command = argv[1];
		start = 2;
	}
...
radiation-results install [--root CHECKOUT] [--check]
radiation-results validate-serial --root CHECKOUT --reference FILE --output DIR [--cells N] [--sanitize]
...
Tools: --gnuplot PATH --ffmpeg PATH --cxx PATH
...
Resume uses saved simulation settings; specify only playback/thread options.
		...
		if (key == "--root")
			...
			throw std::runtime_error("--visit was replaced by --gnuplot; movies now read Silo directly");
		else
	...
	if (o.command == "live" || o.command == "run") {
		...
		if (std::set<std::string>{"debug", "release", "relwithdebinfo"}.contains(build_name)) {
			std::string canonical = build_name == "debug"	  ? "Debug"
									: build_name == "release" ? "Release"
															  : "RelWithDebInfo";
			if (fs::is_directory(o.root / build_name))
				o.build = build_name;
		...
		if (o.command == "run")
			o.no_movies = true;
	} else if (o.command == "plot" || o.command == "pages" || o.command == "movies" || o.command == "check") {
