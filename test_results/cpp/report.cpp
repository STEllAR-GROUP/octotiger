...
{
	std::string s =
		"<div class=\"scroll\"><table><thead><tr><th>Field</th><th>Q(0)</th><th>Q(t)</th><th>Boundary "
		"B</th><th>Source S</th><th>Residual R</th><th>Final |R|/scale</th><th>Maximum "
		"|R|/scale</th></tr></thead><tbody>";
	for (auto &r : b.summary) {
		s += "<tr><th scope=\"row\">" + show(r.at("field")) + "</th>";
		for (auto key : {"initial", "final", "boundary", "source", "residual", "normalized_error",
...
<p><var>B</var> is outward transport and <var>S</var> is the source change. Energy integrals have units erg in CGS; physical-flux integrals have units erg cm/s. Dividing a flux integral by <var>c</var>² gives radiation momentum. Energy residuals are normalized by max(|<var>E</var><sub>0</sub>|, |<var>E</var>|, |<var>B</var><sub>E</sub>|, |<var>S</var><sub>E</sub>|). A flux component uses the maximum of its corresponding terms and <var>c</var> times that energy scale, so a zero net flux does not cause division by zero. Summaries show the final residual and the largest normalized residual over the saved history.</p>
	...
	for (auto &d : problem_descriptions) {
		...
		if (runs.empty()) {
			...
			records_csv(case_dir / "conservation.csv", budgets,
						{"case", "level", "cells_per_side", "time", "field", "initial", "final", "boundary",
						 "source", "residual", "normalized_error", "max_normalized_error", "integral_units"});
